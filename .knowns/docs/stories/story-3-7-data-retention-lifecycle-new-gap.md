---
title: 'Story 3.7: Data Retention & Lifecycle (New Gap)'
description: ''
createdAt: '2026-07-28T10:28:33.078Z'
updatedAt: '2026-07-28T15:17:33.370Z'
tags:
  - bmad
  - bmad-source-bmad-output-implementation-artifacts-3-7-data-retention-lifecycle-new-gap-md
---
...
- [ ] **Backend schema & migration** (AC 2, 3)
  - [ ] Add `DocumentRetentionAction` enum and new columns to `Workspace` and `Document` in `app/db.py`
  - [ ] Create `nowing_backend/alembic/versions/176_add_document_retention.py` (revision `176`, down_revision `175`) and reconcile `zero_publication`
  - [ ] Add DB indexes for `Workspace(auto_archive_enabled)` and `Document(archived_at, workspace_id)`
- [ ] **Backend schemas & routes** (AC 2, 3, 7)
  - [ ] Update `WorkspaceUpdate`, `WorkspaceRead`, `WorkspaceWithStats` build in `workspaces_routes.py`
  - [ ] Update `DocumentRead` and `DocumentWithChunksRead` to include `archived_at`
  - [ ] Validate retention fields in `update_workspace`
  - [ ] Add `archived_at.is_(None)` filters to `read_documents`, `search_documents`, `search_document_titles`, `get_document_type_counts`, `get_document_by_chunk_id`, `get_document_by_virtual_path`, and `read_document`
- [ ] **Hybrid search / citation guardrails** (AC 5)
  - [ ] Update base conditions in `documents_hybrid_search.py` and `chunks_hybrid_search.py`
- [ ] **Celery lifecycle task** (AC 4)
  - [ ] Create `nowing_backend/app/tasks/celery_tasks/document_retention_task.py`
  - [ ] Register the task in `app/celery_app.py` `include` list and `beat_schedule`
- [ ] **Web UI — Data retention tab** (AC 1, 2)
  - [ ] Add `data-retention` tab to `WorkspaceSettingsLayoutShell` and `messages/en.json`
  - [ ] Create `app/dashboard/[workspace_id]/workspace-settings/data-retention/page.tsx`
  - [ ] Create `components/settings/data-retention-manager.tsx`
  - [ ] Extend `workspace.types.ts`, `workspaces-api.service.ts`, `workspace-mutation.atoms.ts`, and `cache-keys.ts` if needed
- [ ] **Zero sync** (AC 6)
  - [ ] Add `archivedAt` to `nowing_web/zero/schema/documents.ts` and `app/zero_publication.py` `DOCUMENT_COLS`
  - [ ] Filter archived docs in `nowing_web/zero/queries/documents.ts`
- [ ] **Tests** (AC 7)
  - [ ] Backend integration tests for `PUT /workspaces/{id}` retention validation and permission checks
  - [ ] Unit/integration tests for the lifecycle task (archive vs delete, multi-tenancy)
  - [ ] Playwright E2E in `tests/workspace-settings/data-retention.spec.ts`
- [ ] **Lint & verify**
  - [ ] `uv run ruff check .` and `uv run pytest tests` in `nowing_backend`
  - [ ] `pnpm lint` / `pnpm format` in `nowing_web`
