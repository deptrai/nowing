---
title: 'Story 8.11: Admin UI for Global LLM Model Configuration'
description: ''
createdAt: '2026-07-28T12:47:48.421Z'
updatedAt: '2026-07-28T15:17:33.678Z'
tags:
  - bmad
  - bmad-source-bmad-output-implementation-artifacts-8-11-admin-ui-global-llm-model-config-md
---
...
- [ ] **T1 — Backend auth boundary** (AC 1)
  - [ ] Add `require_superuser()` in `nowing_backend/app/users.py`; depend on `require_session_context()` and check `auth.user.is_superuser`.
  - [ ] Add unit/integration tests proving non-superuser sessions and PATs get 403 and superuser sessions pass.
  - [ ] Keep workspace RBAC helpers unchanged; do not add Admin workspace role.

- [ ] **T2 — Admin schemas and source labels** (AC 2, 4, 7)
  - [ ] Extend or add Pydantic schemas in `nowing_backend/app/schemas/model_connections.py` for admin global reads/writes.
  - [ ] Include source labels: `managed` for DB-backed rows, `file` or `config` for YAML/env/OpenRouter virtual rows.
  - [ ] Include pricing/rpm/tpm fields in a structured shape; normalize secrets so reads always return `api_key=None`.
  - [ ] Update `nowing_web/contracts/types/model-connections.types.ts` with matching Zod contracts.

- [ ] **T3 — Dedicated admin API routes** (AC 1, 2, 3, 4, 6, 8, 11)
  - [ ] Add `/admin/global-model-connections*` routes, mounted under the existing `/api/v1` router.
  - [ ] Reuse `_connection_read`, `_selection_to_model`, `_apply_model_facts`, `discover_models`, `verify_connection`, and `test_model` where valid.
  - [ ] Ensure regular `/model-connections*` routes still reject `scope=GLOBAL` and still require workspace/user ownership.
  - [ ] Reject write attempts against file-backed virtual IDs.
  - [ ] Add structured logs for all admin operations.

- [ ] **T4 — Managed GLOBAL persistence and migration decision** (AC 3, 4, 9)
  - [ ] Persist managed rows as `Connection.scope=GLOBAL`, `workspace_id=None`, `user_id=None`.
  - [ ] Store cost/rpm/tpm/quality metadata in `Model.catalog` and/or `billing_tier`; keep `Connection.extra` for provider/LiteLLM kwargs.
  - [ ] Decide whether a migration is required. If adding columns/tables, create Alembic migration after revision 179 and include downgrade. If no migration is needed, add an explicit no-migration note in code review/dev notes and cover existing constraint behavior in tests.

- [ ] **T5 — DB-backed global catalog merge** (AC 2, 3, 4, 5, 7, 9)
  - [ ] Extend `materialize_global_model_catalog()` or introduce a helper it calls to merge enabled/disabled DB GLOBAL `Connection`/`Model` rows.
  - [ ] Preserve YAML/env and OpenRouter precedence; append managed entries with deterministic stable runtime IDs.
  - [ ] Add source metadata and DB IDs to `catalog` without exposing secrets.
  - [ ] Ensure disabled managed connections/models are not usable by Auto mode.
  - [ ] Update `_global_catalog_has_usable_chat()` tests for managed entries.

- [ ] **T6 — Hot refresh helper** (AC 5, 7, 8, 9, 11)
  - [ ] Add a single post-commit helper that serializes refresh with a process-local lock.
  - [ ] Call `refresh_global_model_catalog()` after successful CRUD and re-run pricing registration.
  - [ ] Rebuild/clear LLM router caches if required for changed global configs to affect router-backed paths.
  - [ ] Ensure refresh failure is logged with committed IDs and returned as a clear server error.

- [ ] **T7 — Pricing registration for managed globals** (AC 7)
  - [ ] Convert per-1k UI values to per-token LiteLLM fields.
  - [ ] Extend `pricing_registration.py` or the catalog/config generation path so managed entries are registered under the same alias rules as YAML entries.
  - [ ] Add tests based on `nowing_backend/tests/unit/services/test_pricing_registration.py`.

- [ ] **T8 — Runtime routing and Auto mode correctness** (AC 5, 9)
  - [ ] Update `auto_model_pin_service` and any direct runtime lookup needed so managed global models are platform/global candidates, not BYOK.
  - [ ] Preserve negative-ID global semantics or update every call site that depends on it.
  - [ ] Add tests for Auto candidate inclusion/exclusion, billing tier, and pinned runtime lookup.

- [ ] **T9 — Frontend platform route and API service** (AC 2, 4, 6, 10)
  - [ ] Add a platform-level route, for example `nowing_web/app/admin/global-model-connections/page.tsx` or an equivalent platform settings route outside `/dashboard/[workspace_id]/...`.
  - [ ] Gate client rendering with `user.is_superuser`, but assume backend auth is authoritative.
  - [ ] Add admin global methods to `nowing_web/lib/apis/model-connections-api.service.ts` and dedicated query/mutation atoms or parameterized model-connection atoms.
  - [ ] Reuse `components/settings/model-connections/*` by extracting endpoint/scope/workspace dependencies; do not call workspace mutations for global CRUD.
  - [ ] Render file-backed rows read-only and managed rows editable.

- [ ] **T10 — Tests and quality gates** (all ACs)
  - [ ] Backend unit tests: `require_superuser`, catalog merge, runtime ID mapping, pricing conversion, refresh failure, secret redaction.
  - [ ] Backend integration tests: admin create/update/delete/test endpoints with superuser and non-superuser, regular route GLOBAL rejection unchanged, no restart catalog refresh.
  - [ ] Frontend unit/component tests: Zod contracts, read-only file-backed rows, managed row edit/delete controls, access denied for non-superuser.
  - [ ] Playwright E2E or route-level test: superuser can add/test/save a managed global model using mocked backend responses; non-superuser cannot reach controls.
  - [ ] Run P0 quality path because this touches auth, provider/model routing, pricing registration, and web UI.
