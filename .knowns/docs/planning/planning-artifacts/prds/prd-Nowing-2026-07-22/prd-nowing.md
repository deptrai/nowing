...
title: 'PRD: Nowing'
description: ''
createdAt: '2026-07-28T12:47:49.339Z'
updatedAt: '2026-07-28T15:17:33.236Z'
tags:
  - bmad
  - bmad-source-bmad-output-planning-artifacts-prds-prd-nowing-2026-07-22-prd-md
---
...
> **[NOTE FOR PM] — Reality-correction 2026-07-24 (đã verify code, sau PRFAQ + validate).** Trái với bản trước, **lớp memory đã được build phần lớn**: migration `177_add_research_memory_tables` tạo `memories`/`memory_versions`/`memory_relations`/`research_threads` (+ enums, `confidence`, index HNSW+GIN, quyền `memory:*`); `179` thêm `workspaces.memory_auto_extract_enabled` (default **true**); ORM `Memory` trong `app/db.py`; endpoints `memories_routes.py`; 4 MCP tools trong `nowing_mcp/.../features/memory/`. PRD gốc chụp trạng thái *trước* khi code đáp (docs đề 2026-07-21; migration 177–179 landing sau). Các FR memory dưới đây đã cập nhật `[BUILT]`/`[PARTIAL]`. **Open items thật (KHÔNG phải "build từ đầu"):** (1) **recall quality/eval gate** trên `nowing_evals` — NFR-8 → **`[PARTIAL — in-progress]`**, story `3-9` = **`in-progress`** (đã lùi từ `review` — **cổng chặn launch**); (2) ~~đánh giá mất dữ liệu (FR-36)~~ → **✅ RESOLVED 2026-07-25: KHÔNG mất dữ liệu** (178 chưa apply prod, alembic 174, `memory_md` rỗng, snapshot đã tạo; guard + backfill + 5 test đã build qua `3-10a`/`3-10b`; giữ deploy-order mig177→backfill→mig178); (3) **dedupe**: primitive đã có (cosine<0.08 + `update_on_duplicate`), cần tune/validate qua eval; (4) legal/retention (OQ-3); (5) metrics; (6) **auto-extract spend cap** — story `8-7` = `ready-for-dev` (default ON, item-cap có, **spend cap chưa**). **Chính xác hoá:** đây là **cổng TRƯỚC KHI merge lên prod**, KHÔNG phải cost bleed đang chạy — migration 175–179 còn ở branch `develop`, prod = `alembic 174` (ops verify 2026-07-25). Chi tiết: `validation-report.md`, `epics.md`.
...
#### FR-24: Deep Open-Web Research via ChainLens Engine  `[PARTIAL — đã wire, thiếu contract-guard]`
...
**Acceptance Criteria:**
- Query dài > 500 ký tự bị clamp trước khi gọi (ChainLens clamp ở 500; `MAX_QUERY_LENGTH`).
- Mọi câu trả lời có `sources[]` giữ nguyên thứ tự trích dẫn để map về citation UI.
- **Mode default = `balanced`** (D3, 2026-07-25). `quality` là **opt-in tường minh** — khi user/agent yêu cầu deep-research hoặc deliverable. Lý do: `quality` = $0.0105/request vs `balanced` = $0.0048 (**2.2×**), và trước 2026-07-25 Nowing âm thầm gọi `quality` cho **mọi** call (`schemas.py:38`). Story 9.3 validate chất lượng trên `nowing_evals` trước khi khoá; reversible qua env.
- Contract regression test tồn tại và chạy trong CI.
...
- `[GAP]` contract regression test (story **9.1b**). `[GAP]` mode default còn `quality` (story 9.3).

#### FR-37: Deep-Research Cost Metering  `[GAP]`
...
- `CHAINLENS_QUERY_MICROS_PER_CALL = 5000` → **$0.005 phẳng mỗi call, bất kể mode** (`app/config/__init__.py:806`, dùng qua `BillingUnit.CHAINLENS_QUERY`).
- Nhưng `mode` default = `"quality"` (`schemas.py:38`), mà target cost của ChainLens là **quality $0.0105 / deep research $0.0164** (ChainLens PRD §7.1) → Nowing **under-meter 2.1×–3.3×**.
- Tệ hơn: hai số đó tính trên **DeepSeek stack chưa vào production** — `DEFAULT_MODEL_POLICY` của ChainLens hiện 100% `ag/` Gemini, output đắt hơn DeepSeek ~3.5×. Hết Gemini runway, gap rộng thêm.
...
#### FR-38: Research Degradation & Self-Host Independence  `[GAP — P0, TIỀN ĐỀ TRƯỚC KHI PUBLIC REPO]`
...
**Gap:**
- `[PARTIAL — in-progress 2026-07-25]` NFR-8: Endpoint `/memories/search` + MCP `nowing_recall` đã có. Eval gate — story `3-9-memory-recall-eval-gate` = **`in-progress`** trong `sprint-status.yaml` (**đã lùi từ `review`**; nếu review tìm ra vấn đề thì đây là việc gấp nhất vì nó chặn launch). Crack đỏ #2 từ PRFAQ **chưa đóng**; còn cần chốt ngưỡng cụ thể cùng SM-10 và xác nhận gate chặn ship thật.
...
**Gap:**
- `[GAP]` NFR-9: chưa có đường async deliverable; chưa đo p50/p95 từ phía Nowing; chưa đặt ngưỡng cổng A→B. Story 9.3.
...
- **Long-term research memory (In Scope — PHẦN LỚN ĐÃ BUILD):** schema + endpoints + 4 MCP tools + hybrid indexes + `confidence` đã có (migration 177–179; `memories_routes.py`; `features/memory/`). **Còn lại (open):** **recall quality eval gate** (NFR-8, story `3-9` = `in-progress` — cổng chặn launch); dedupe tuning qua eval; **auto-extract spend cap** (story `8-7` = `ready-for-dev` — cổng trước merge); memory type ngoài semantic (defer). ~~đánh giá mất dữ liệu legacy (FR-36)~~ → **đã đóng 2026-07-25, không mất dữ liệu.** Auto-extract đã có (179, default true) — review chi phí/ngân sách, không phải "chưa build".
...
- **Deep-Research Engine Integration (§4.9, Epic 9)** — ChainLens là engine deep-open-web-research phía sau Nowing, không phải một connector. In scope MVP: **FR-38** degradation + self-host independence (`[GAP]`, **P0 — tiền đề trước khi public repo**, story `9.1a`, chạy trước FR-24/FR-37); **FR-37** cost metering thật qua `costDollars` (`[GAP]`, P0); **FR-24** contract + regression guard (`[PARTIAL]` — đã wire, thiếu guard); **NFR-9 State A** đường async deliverable + đo p50/p95 từ phía Nowing (`[GAP]`). Mode default đổi `quality` → `balanced` (D3).
...
#### OQ-6: Đồng bộ docs & artifacts với vision mới  `[PARTIAL — 2026-07-25]`
README, `docs/`, và `docs/project-overview.md` vẫn ở trạng thái **pre-pivot** ("NotebookLM alternative / open web research"), chưa phản ánh "long-term research memory". Cần kế hoạch đồng bộ trước khi cam kết vision công khai — giảm rủi ro nhận thức "pivot chưa ổn định".

- ✅ **Đã đóng:** `_bmad-output/planning-artifacts/epics.md` **đã tồn tại** (tạo 2026-07-25).
- 🟠 **Còn mở:** public docs vẫn pre-pivot. **Mở rộng phạm vi 2026-07-25** — docs giờ phải nói thêm **quan hệ Nowing ↔ ChainLens** (Nowing = sản phẩm, ChainLens = engine; hành vi khi không cấu hình ChainLens theo FR-38; self-host setup trong `docker/` + `.env.example`).

**Gap:** `[PARTIAL]` OQ-6 — `epics.md` đã có; public docs pre-pivot + thiếu mục Nowing↔ChainLens. Story **9.4** (AR-10 mở rộng).
