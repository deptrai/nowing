...
export class StatusBar {
	...
	constructor(host: HTMLElement, onClick?: () => void) {
		this.el = host;
		this.el.addClass("nowing-status");
		this.icon = this.el.createSpan({ cls: "nowing-status__icon" });
		this.text = this.el.createSpan({ cls: "nowing-status__text" });
		if (onClick) {
			this.el.addClass("nowing-status--clickable");
	...
	update(state: StatusState): void {
		...
		this.el.removeClass("nowing-status--err");
		if (visual.isError) this.el.addClass("nowing-status--err");
		setIcon(this.icon, visual.icon);

		let label = `Nowing: ${visual.label}`;
