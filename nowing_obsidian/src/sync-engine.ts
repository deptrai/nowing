...
export class SyncEngine {
	...
	async start(): Promise<void> {
		this.setStatus("syncing", "Connecting to Nowing…");
		...
		if (upserts.length > 0) {
			...
			for (const item of upserts) {
				...
				try {
					...
					console.error("Nowing: failed to build payload", item.path, err);
					retry.push(item);
				}
			}
	...
	private async handleVaultNotRegistered(err: unknown): Promise<boolean> {
		...
		console.warn("Nowing: vault not registered, re-connecting before retry", err);
		await this.ensureConnected();
		return true;
	}
	...
	private classify(err: unknown): "ack" | "retry" | "drop" | "stop" {
		...
		if (err instanceof PermanentError) {
			console.warn("Nowing: permanent error, dropping batch", err);
			new Notice(`Nowing: ${err.message}`);
			return "drop";
		}
		console.error("Nowing: unknown error", err);
		return "retry";
	}
