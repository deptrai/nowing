...
import { NowingApiClient } from "./api-client";
import { PersistentQueue } from "./queue";
import { NowingSettingTab } from "./settings";
...
import {
	...
	type QueueItem,
	type StatusState,
	type NowingPluginSettings,
} from "./types";
import { generateVaultUuid } from "./vault-identity";

/** Nowing plugin entry point. */
export default class NowingPlugin extends Plugin {
	settings!: NowingPluginSettings;
	api!: NowingApiClient;
	...
	private settingTab: NowingSettingTab | null = null;
	...
	async onload() {
		...
		this.settingTab = new NowingSettingTab(this.app, this);
		...
		this.addCommand({
			...
			callback: async () => {
				try {
					...
					new Notice("Nowing: re-sync started.");
				} catch (err) {
					new Notice(`Nowing: re-sync failed — ${(err as Error).message}`);
				}
			},
		});
	...
	private notifyAuthError(): void {
		...
		new Notice("Nowing: API token is invalid or expired. Check your token in settings.", 8000);
	}
	...
	async loadSettings() {
		...
		const data = (await this.loadData()) as
			| (Partial<NowingPluginSettings> & { searchSpaceId?: number | null })
			| null;
