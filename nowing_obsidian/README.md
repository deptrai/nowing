...
Sync your Obsidian vault to [Nowing](https://github.com/nowing/Nowing)
so your notes become searchable alongside the rest of your knowledge sources
(GitHub, Slack, Linear, Drive, web pages, etc.) from any Nowing chat.

The plugin runs inside Obsidian itself, on desktop and mobile, so it works
the same way for Nowing Cloud and self-hosted deployments. There is no
...
- Multi-vault aware: each vault you enable the plugin in becomes its own
  connector row in Nowing, named after the vault
...
3. Paste `nowing/Nowing` and pick the latest release.
4. Enable **Nowing** in *Settings → Community plugins*.
...
2. Copy them into `<vault>/.obsidian/plugins/nowing/`.
...
Open **Settings → Nowing** in Obsidian and fill in:

| Setting | Value |
| --- | --- |
| Server URL | `https://nowing.com` for Nowing Cloud, or your self-hosted URL |
| API token | Create a personal access token from the *Connectors → Obsidian* dialog or *User settings → API access* in the Nowing web app |
...
The connector row appears automatically inside Nowing the first time the
...
The Nowing backend qualifies as server-side telemetry under Obsidian's
...
canonical Nowing privacy policy lives at
<https://nowing.com/privacy>; this section is the plugin-specific
addendum.
...
- `workspace_id`: the Nowing workspace you picked
...
<https://nowing.com/privacy>.

## Development

This plugin lives in [`nowing_obsidian/`](.) inside the Nowing
...
Symlink the folder into a test vault's `.obsidian/plugins/nowing/`,
...
[Apache-2.0](LICENSE), same as the rest of Nowing.
