...
and Nowing). That makes per-question outcomes paired, so
...
class McnemarResult:
    ...
    n_total: int
    b: int  # native correct, nowing wrong
    c: int  # native wrong,   nowing correct
    statistic: float
    p_value: float
    method: str
...
def mcnemar_test(
    ...
    *,
    ...
    "nowing"). The test statistic only depends on discordant pairs.
