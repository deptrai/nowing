"""Argparse CLI for ``python -m nowing_evals``.
...
def _resolve_openrouter_id(
    ...
    *,
    ...
    """Resolve the Nowing LLM id for ``provider_model``.
    ...
    * Otherwise: filter to ``provider == "OPENROUTER"`` and
    ...
    if not matches:
        ...
        raise RuntimeError(
            ...
            "Browse with: python -m nowing_evals models list --grep <substring>"
        )
    if len(matches) > 1:
...
async def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    if scenario == "cost-arbitrage":
        if not native_arm_model:
            console.print(
                ...
                "model to fairly answer image-bearing questions; Nowing "
    ...
    async with client_with_auth(config, token) as http:
        candidates = await _list_global_llm_configs(http, config.nowing_api_base)
        ...
        ss_client = SearchSpaceClient(http, config.nowing_api_base)
        ...
        # requirement — without it, Nowing's chunks have no image
...
async def _cmd_models_list(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 2
    async with client_with_auth(config, token) as http:
        entries = await _list_global_llm_configs(http, config.nowing_api_base)
...
def _cmd_suites_list(_args: argparse.Namespace) -> int:
    ...
    if not suites:
        console.print(
            ...
            "src/nowing_evals/suites/<domain>/<benchmark>/.[/yellow]"
        )
        return 0
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="nowing-evals",
        description="Nowing evaluation harness — domain-agnostic core + pluggable suites.",
    )
    parser.add_argument(
    ...
    p_setup.add_argument(
        ...
        required=True,
        help=(
            "OpenRouter slug for the Nowing answer LLM (and the native arm "
    ...
    p_setup.add_argument(
        ...
        help=(
            ...
            "Nowing pre-extracted images at ingest with a vision LLM; "
            "cost-arbitrage: native arm uses --native-arm-model (vision), "
            "Nowing uses --provider-model (cheap, text-only) over chunks "
    ...
    p_setup.add_argument(
        ...
        metavar="SLUG",
        help=(
            "OpenRouter slug for the vision LLM Nowing uses at ingest "
    ...
    p_setup.add_argument(
        ...
        help=(
            ...
            "by the native_pdf arm only; Nowing answers with "
