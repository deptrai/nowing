...
Nowing supports ``AUTH_TYPE=LOCAL`` (email + password) and
...
* **GOOGLE / pre-issued JWT**: operator pastes their existing JWT (and
  optionally refresh token) into ``NOWING_JWT`` /
  ``NOWING_REFRESH_TOKEN``; harness skips login.
...
_NO_CREDENTIALS_MESSAGE = (
    "No Nowing credentials configured. Set ONE of:\n"
    "  (LOCAL)  NOWING_USER_EMAIL + NOWING_USER_PASSWORD\n"
    "  (GOOGLE) NOWING_JWT (and optionally NOWING_REFRESH_TOKEN)\n"
    "For GOOGLE: use a PAT or operator-issued bearer token and set "
    "NOWING_JWT (plus NOWING_REFRESH_TOKEN if available)."
)
...
async def acquire_token(config: Config, *, http: httpx.AsyncClient | None = None) -> TokenBundle:
    ...
    1. ``NOWING_JWT`` set → use it directly. Refresh token captured if
       supplied.
    2. ``NOWING_USER_EMAIL`` + ``NOWING_USER_PASSWORD`` set →
    ...
    if config.has_jwt_mode():
        return TokenBundle(
            access_token=config.nowing_jwt or "",
            refresh_token=config.nowing_refresh_token,
            mode="jwt",
        )
    ...
    if config.has_local_mode():
        ...
        async def _login(client: httpx.AsyncClient) -> TokenBundle:
            response = await client.post(
                f"{config.nowing_api_base}/auth/desktop/login",
                json={
                    "email": config.nowing_user_email,
                    "password": config.nowing_user_password,
                },
    ...
    try:
        async with httpx.AsyncClient(
            ...
            response = await inner.post(
                f"{state.config.nowing_api_base}/auth/jwt/refresh",
...
def client_with_auth(
    ...
    *,
    ...
    """Build a single shared ``httpx.AsyncClient`` for the Nowing API.
