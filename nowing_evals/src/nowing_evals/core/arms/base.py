...
* ``NativePdfArm`` — drop the PDF straight into an OpenRouter
  ...
  its own?" measurement.
* ``NowingArm`` — POST ``/api/v1/new_chat`` with the question
...
class ArmRequest:
    ...
    * ``pdf_paths`` is the per-question source PDFs (used by
      ...
      benchmarks.
    * ``mentioned_document_ids`` is the Nowing document scoping list
      (used by ``NowingArm``). When ``None`` Nowing retrieves
      across the whole search space.
    * ``options`` is a free-form bag of arm-specific overrides
      (e.g. Nowing's ``disabled_tools``).
    """
