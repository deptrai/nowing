...
The extensibility seam: ``core.cli`` walks ``nowing_evals.suites`` on
...
class RunContext:
    ...
    ``http`` is the authenticated Nowing client (auth event hook
    ...
    def provider_model(self) -> str:
        """Slug used by the Nowing agent (and the native arm by default).

        For ``cost-arbitrage`` scenarios this is the *cheap, text-only*
        slug — Nowing answers from the chunks the vision LLM already
    ...
    def vision_provider_model(self) -> str | None:
        """Slug of the OpenRouter vision LLM Nowing used at ingest.
    ...
    def search_space_client(self) -> SearchSpaceClient:
        return SearchSpaceClient(self.http, self.config.nowing_api_base)
