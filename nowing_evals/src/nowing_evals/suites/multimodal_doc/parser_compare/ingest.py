...
* ``document_id``    — Nowing document id (carried over from
                        mmlongbench's existing ingestion so the
                        Nowing agentic arm can scope retrieval).
...
arm, and uses ``document_id`` for the Nowing arm.
...
parser quality on the same physical PDFs Nowing already has, so
...
async def run_ingest(
    ...
    *,
    ...
    # ingestion. parser_compare doesn't re-upload; Nowing's agentic
