...
* ``build_nowing_prompt``        — bare question + format hint
...
def build_nowing_prompt(question: str, *, answer_format: str) -> str:
    """Prompt for ``NowingArm`` — chunks retrieved by the agent."""

    # Nowing's agent already injects retrieved chunks via its tool
    # loop; the prompt only carries the user-visible question + format
    # hint, mirroring how a human asks the Nowing UI.
    return f"{_BASE_INSTRUCTION}\n\nQuestion: {question.strip()}\n\n{_format_hint(answer_format)}\n"
