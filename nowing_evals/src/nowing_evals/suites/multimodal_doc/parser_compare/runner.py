...
* ``nowing_agentic``    — Nowing /api/v1/new_chat retrieval over chunks.

Cost reporting:

* ``llm_cost_per_q``       — mean OpenRouter ``usage.cost`` reported by
                              the chat-completions API. Zero for the
                              Nowing agentic arm because the SSE
...
# The Nowing agentic arm is fed by the existing mmlongbench
...
_DESCRIPTION = (
    ...
    "stuffing + Nowing agentic retrieval. Reports preprocessing dollars "
...
class ParserCompareBenchmark:
    ...
    def add_run_args(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            dest="no_mentions",
            action="store_true",
            help="Nowing arm: skip mentioned_document_ids (full-corpus retrieval).",
        )
        parser.add_argument(
            "--pdf-engine",
            default="native",
        ...
        parser.add_argument(
            ...
            "Nowing arm uses whatever provider_model is pinned on the suite.",
        )
        parser.add_argument(
            "--skip-arms",
            default=None,
    ...
    async def run(self, ctx: RunContext, **opts: Any) -> RunArtifact:
        ...
        if not map_path.exists():
            raise RuntimeError(
                ...
                "`python -m nowing_evals ingest multimodal_doc parser_compare` first."
            )
        ...
        if "nowing_agentic" in active_arms:
            surf = NowingArm(
                client=ctx.new_chat_client(),
                search_space_id=ctx.search_space_id,
                ephemeral_threads=True,
            )
            # Override the default "nowing" name so the metrics
            # bucket lines up with the rest of parser_compare's arms.
            surf.name = "nowing_agentic"
            arms["nowing_agentic"] = surf
        ...
        def _surf_req(q: PCQuestion) -> ArmRequest:
            ...
            return ArmRequest(
                question_id=q.qid,
                prompt=build_nowing_prompt(q.question, answer_format=q.answer_format),
    ...
    def report_section(self, artifacts: list[RunArtifact]) -> ReportSection:
        ...
        body.append("")
        body.append(
            "- `nowing_agentic` LLM cost shows as $0.0000 because the "
...
def _compute_metrics(
    ...
    for arm_name in active_arms:
        ...
        elif arm_name == "nowing_agentic":
            preprocess_per_page = PREPROCESS_USD_PER_PAGE[NOWING_INGEST_MODE]
            preprocess_label = (
                f"{NOWING_INGEST_MODE} tier (ingested by Nowing at "
