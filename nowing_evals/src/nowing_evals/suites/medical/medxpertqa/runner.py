"""MedXpertQA-MM runner — Native PDF (vision) vs Nowing (vision RAG).
...
* Nowing arm queries ``POST /api/v1/new_chat`` scoped via
...
class MedXpertQAMMBenchmark:
    ...
    def add_run_args(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            dest="skip_upload",
            action="store_true",
            help="(ingest only) render PDFs locally but don't push to Nowing.",
        )
        parser.add_argument(
            "--include-dev",
            dest="include_dev",
    ...
    async def run(self, ctx: RunContext, **opts: Any) -> RunArtifact:
        ...
        if not questions_jsonl.exists() or not map_path.exists():
            raise RuntimeError(
                ...
                "`python -m nowing_evals ingest medical medxpertqa` first."
            )
    ...
    def report_section(self, artifacts: list[RunArtifact]) -> ReportSection:
        if not artifacts:
            return ReportSection(
                title="MedXpertQA-MM — Native PDF (vision) vs Nowing (vision RAG)",
                headline=False,
        ...
        if per_task:
            ...
            for task_name, vals in sorted(per_task.items()):
                body_lines.append(
                    f"  - {task_name}: Nowing {_pp(vals.get('delta_accuracy_pp'))} pp "
                    f"(n={vals.get('n')})"
                )
        if per_body:
            ...
            for body_name, vals in top:
                body_lines.append(
                    f"  - {body_name}: Nowing {_pp(vals.get('delta_accuracy_pp'))} pp "
                    f"(n={vals.get('n')})"
                )
...
def _make_nowing_request(q: MXQuestion, *, no_mentions: bool) -> ArmRequest:
    prompt = build_prompt(q.question, q.options)
