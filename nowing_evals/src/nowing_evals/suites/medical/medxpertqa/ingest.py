...
4. Upload each PDF to Nowing with ``use_vision_llm=True``; persist
   ``id -> document_id`` in
   ``<data_dir>/medical/maps/medxpertqa_doc_map.jsonl``.

Both arms then receive byte-identical PDFs. The native arm sends the
PDF directly to OpenRouter; Nowing ingests via its own vision
...
async def run_ingest(
    ...
    *,
    ...
    skip_upload : bool
        Render PDFs locally but don't push to Nowing.
    include_dev : bool
    ...
    name_to_id: dict[str, int] = {}
    if skip_upload:
        logger.info("MedXpertQA: --skip-upload set; skipping Nowing ingestion")
    else:
