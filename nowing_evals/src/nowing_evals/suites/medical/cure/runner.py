"""CUREv1 runner — single-arm Nowing retrieval scoring.

For each query we ask Nowing via ``/api/v1/new_chat`` (no
...
Nowing's standard ``[citation:CHUNK_ID]`` format (already required
...
_DESCRIPTION = "CUREv1 retrieval (single-arm Nowing): Recall@k / MRR / nDCG@10."
    ...
    async def run(self, ctx: RunContext, **opts: Any) -> RunArtifact:
        ...
        if not chunk_to_doc or not doc_to_corpus:
            raise RuntimeError(
                ...
                "`python -m nowing_evals ingest medical cure` first."
            )
    ...
    def report_section(self, artifacts: list[RunArtifact]) -> ReportSection:
        if not artifacts:
            return ReportSection(
                title="CUREv1 — single-arm Nowing retrieval",
                headline=False,
