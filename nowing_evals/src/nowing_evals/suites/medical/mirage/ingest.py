...
``~5 MB`` markdown bundles for Nowing's file upload (smaller
...
* ``mirage_chunk_map.jsonl`` — ``{chunk_id, document_id, snippet_id?}``
  ...
  snippet it overlaps when the Nowing chunker splits a batched
  markdown).
"""
...
async def run_ingest(
    ...
    *,
    ...
    chosen corpus and let Nowing's own retriever do the work. The
    ...
    # Best-effort chunk map. Nowing doesn't expose snippet attribution
