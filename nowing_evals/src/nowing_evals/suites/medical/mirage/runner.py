"""MIRAGE runner: Nowing-only per-task accuracy.
...
_DESCRIPTION = "MIRAGE (7,663 medical MCQs) — single-arm Nowing per-task accuracy."
    ...
    async def run(self, ctx: RunContext, **opts: Any) -> RunArtifact:
        ...
        if not bench_path.exists():
            raise RuntimeError(
                ...
                "`python -m nowing_evals ingest medical mirage` first."
            )
    ...
    def report_section(self, artifacts: list[RunArtifact]) -> ReportSection:
        if not artifacts:
            return ReportSection(
                title="MIRAGE — single-arm Nowing per-task accuracy",
                headline=False,
