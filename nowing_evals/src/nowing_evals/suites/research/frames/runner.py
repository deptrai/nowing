"""FRAMES runner — Bare LLM (no retrieval) vs Nowing (multi-hop RAG).
...
1. ``BareLlmArm``  — OpenRouter chat completion with the question only.
   ...
   Gemini-Pro-1.5).
2. ``NowingArm`` — POST ``/api/v1/new_chat`` with **no**
...
from .prompt import build_bare_prompt, build_nowing_prompt
...
_DESCRIPTION = (
    ...
    "Bare LLM (no retrieval) vs Nowing (multi-step RAG over the "
...
class FramesBenchmark:
    ...
    def add_run_args(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            action="store_true",
            help=(
                "Nowing arm: scope retrieval to the per-question "
        ...
        parser.add_argument(
            ...
            dest="skip_upload",
            action="store_true",
            help="(ingest only) cache wiki articles locally but don't push to Nowing.",
        )
        parser.add_argument(
            "--fetch-rps",
    ...
    async def run(self, ctx: RunContext, **opts: Any) -> RunArtifact:
        ...
        if not questions_jsonl.exists() or not map_path.exists():
            raise RuntimeError(
                ...
                "`python -m nowing_evals ingest research frames` first."
            )
        ...
        async def _surf_one(q: FramesRunnerQuestion) -> ArmResult:
            return await surf_arm.answer(_make_nowing_request(q, scope_mentions=scope_mentions))
    ...
    def report_section(self, artifacts: list[RunArtifact]) -> ReportSection:
        if not artifacts:
            return ReportSection(
                title="FRAMES — Bare LLM vs Nowing (multi-hop Wikipedia RAG)",
                headline=True,
        ...
        body_lines.append(
            "- Nowing arm (`POST /api/v1/new_chat`, multi-step RAG, "
        ...
        if per_reasoning:
            ...
            for tag, vals in sorted(per_reasoning.items()):
                body_lines.append(
                    f"  - {tag}: Nowing {_pp(vals.get('delta_accuracy_pp'))} pp "
...
def _make_nowing_request(q: FramesRunnerQuestion, *, scope_mentions: bool) -> ArmRequest:
    mentions: list[int] | None = None
    if scope_mentions and q.document_ids:
        mentions = list(q.document_ids)
    return ArmRequest(
        question_id=q.qid,
        prompt=build_nowing_prompt(q.question),
...
def _compute_metrics(
    ...
    grader_methods = {
        ...
        "nowing": _count_methods(surf_grades),
    }
