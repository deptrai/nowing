...
6. Upload the resulting markdown files to Nowing in batches with
...
We resolve titles → Nowing document_ids via the post-upload
``DocumentStatus.title`` field. Nowing's title is the uploaded
...
async def _upload_markdowns(
    ...
    *,
    ...
    Nowing dedupes uploads on ``(filename, search_space_id)``, so
    ...
    for batch_start in range(0, len(paths), batch_size):
        ...
        if all_ids:
            ...
            for s in statuses:
                # Nowing stores the uploaded filename as ``title`` (no extension).
...
async def run_ingest(
    ...
    *,
    ...
    skip_upload : bool
        ...
        Nowing. Useful for debugging the fetcher in isolation.
    ...
    # 4. Upload to Nowing (deduped by filename).
    name_to_id: dict[str, int] = {}
    if skip_upload:
        logger.info("FRAMES: --skip-upload; skipping Nowing ingestion")
    else:
