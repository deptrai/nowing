...
4. For each question, extract the 5 web pages to clean markdown via
   ...
   filename is unique across the whole sample (so Nowing's
...
5. Upload all extracted pages to Nowing in batches with text-only
...
The runner uses ``document_ids`` to scope Nowing retrieval to
...
def _materialise_pages(
    ...
    *,
    ...
    Nowing's chunker an empty file.
    """
...
async def run_ingest(
    ...
    *,
    ...
    skip_upload
        Extract + cache markdown locally but don't push to Nowing
    ...
    name_to_id: dict[str, int] = {}
    if skip_upload:
        logger.info("CRAG: --skip-upload; skipping Nowing ingestion")
    else:
