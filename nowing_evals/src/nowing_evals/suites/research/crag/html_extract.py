...
2. If extraction fails or returns < 200 chars (paywalled / JS-only
   ...
   text is better than no text — Nowing's chunker handles noisy
   prose.

We *intentionally* keep the page name and URL as visible H1 / link
metadata so the Nowing chunker preserves doc identity at the top of
...
class _StripHTMLParser(HTMLParser):
    ...
    noise Nowing ends up retrieving against if not removed.
    """
...
def extract_main_content(
    ...
    * Nowing's chunker has a stable doc-identity anchor at the top
