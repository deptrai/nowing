...
* ``build_nowing_prompt(q)``    — question + a hint that retrieval
  over the question's 5 ingested pages is available; the Nowing
...
def build_nowing_prompt(question: str, *, query_time: str = "") -> str:
    """Prompt for the Nowing arm (agent does retrieval itself)."""

    return _NOWING_TEMPLATE.format(
        instructions=_BASE_INSTRUCTIONS,
        question=question.strip(),
        query_time=query_time.strip() or "unknown",
    )
