"""CRAG runner — Bare LLM vs Long-Context LLM vs Nowing.
...
3. ``nowing``     — POST ``/api/v1/new_chat`` with retrieval scoped
...
flag for each arm pair (long_context vs bare, nowing vs
long_context, nowing vs bare) so the report can call out exactly
where the lift comes from. Per-domain and per-question-type breakdowns
surface where Nowing beats long-context (e.g. multi-hop / set
...
from ....core.arms import ArmRequest, ArmResult, BareLlmArm, NowingArm
...
_DESCRIPTION = (
    ...
    "arms (Bare LLM / Long-Context LLM / Nowing) over the same "
...
class CragBenchmark:
    """3-arm CRAG runner: bare vs long-context vs Nowing."""
    ...
    def add_run_args(self, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            dest="skip_nowing",
            action="store_true",
            help="Skip the Nowing arm (useful when iterating on the LLM arms only).",
        )
        parser.add_argument(
            "--no-mention-scope",
            ...
            action="store_true",
            help=(
                "Nowing arm: don't pass mentioned_document_ids; let "
        ...
        parser.add_argument(
            ...
            dest="skip_upload",
            action="store_true",
            help="(ingest only) extract pages locally but don't push to Nowing.",
        )
        parser.add_argument(
            "--overwrite-extract",
    ...
    async def run(self, ctx: RunContext, **opts: Any) -> RunArtifact:
        ...
        skip_nowing = bool(opts.get("skip_nowing"))
        ...
        if not map_path.exists():
            raise RuntimeError(
                ...
                "`python -m nowing_evals ingest research crag --n-questions 200` first."
            )
        ...
        async def _surf_one(q: CragRunnerQuestion) -> ArmResult:
            ...
            return await surf_arm.answer(
                _make_nowing_request(q, scope_to_pages=not no_mention_scope)
            )
        ...
        if surf_arm is not None:
            ...
            tasks.append(_make_skipped_results(questions, "nowing"))
    ...
    def report_section(self, artifacts: list[RunArtifact]) -> ReportSection:
        if not artifacts:
            return ReportSection(
                title="CRAG — Bare LLM vs Long-Context LLM vs Nowing",
                headline=True,
        ...
        if not active.get("nowing", True):
            body_lines.append("- Nowing arm: SKIPPED.")
        else:
            body_lines.append(
                f"- Nowing arm (`{extra.get('provider_model', '?')}`, retrieval over "
        ...
        if deltas:
            ...
            for label, key in (
                ("Nowing vs Bare", "nowing_vs_bare"),
                ("Nowing vs Long-Context", "nowing_vs_long_context"),
        ...
        if per_domain:
            ...
            for domain in sorted(per_domain.keys()):
                ...
                for arm in ("bare_llm", "long_context", "nowing"):
                    if arm not in row:
                        continue
...
def _make_nowing_request(q: CragRunnerQuestion, *, scope_to_pages: bool) -> ArmRequest:
    mentions: list[int] | None = None
    if scope_to_pages and q.document_ids:
        mentions = list(q.document_ids)
    return ArmRequest(
        question_id=q.qid,
        prompt=build_nowing_prompt(q.question, query_time=q.query_time),
...
def _compute_metrics(
    *,
    ...
    out: dict[str, Any] = {
        ...
        "nowing": _arm_block(surf_results, surf_acc, surf_t),
    }
    ...
    for label, ref_correct, ref_t, chal_correct, chal_t, both_active in (
        (
            ...
            surf_correct,
            surf_t,
            arms_active.get("bare_llm") and arms_active.get("nowing"),
        ),
        (
            "nowing_vs_long_context",
            lc_correct,
            lc_t,
            surf_correct,
            surf_t,
            arms_active.get("long_context") and arms_active.get("nowing"),
        ),
        (
    ...
    out["grader_methods"] = {
        ...
        "nowing": _count_methods(surf_grades) if arms_active.get("nowing") else {},
    }
    return out
...
def _per_facet_truthfulness(
    ...
    *,
    ...
    for q, b, lc, s in zip(questions, bare_grades, lc_grades, surf_grades, strict=False):
        key = key_fn(q)
        bucket = buckets.setdefault(key, {"bare_llm": [], "long_context": [], "nowing": []})
        bucket["bare_llm"].append(b)
        bucket["long_context"].append(lc)
        bucket["nowing"].append(s)
    out: dict[str, Any] = {}
