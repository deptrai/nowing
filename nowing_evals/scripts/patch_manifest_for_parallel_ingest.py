...
The mmlongbench Nowing ingest writes its manifest only at the very
...
Nowing ``document_id`` (the runner does, later, after a refresh).
...
Nowing ``document_id``s yet) so parser_compare can run all four
extractions for them in parallel with the Nowing ingest.

After mmlongbench finishes, re-run::

    python -m nowing_evals ingest multimodal_doc parser_compare \
        --max-docs 30
...
def main() -> None:
    if not MAP_PATH.exists():
        raise SystemExit(
            ...
            "run `nowing_evals ingest multimodal_doc mmlongbench` first."
        )
