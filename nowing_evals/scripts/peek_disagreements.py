"""Show questions where Nowing was wrong but long-context was right (and vice versa)."""
...
def main() -> None:
    ...
    for qid, arms in by_q.items():
        ...
        s = arms.get("nowing", {}).get("graded", {}).get("grade")
    ...
    print(f"\nNowing INCORRECT but Long-Context CORRECT: {len(surf_wrong_lc_right)}")
    print(f"Long-Context INCORRECT but Nowing CORRECT: {len(lc_wrong_surf_right)}")
    print(f"Nowing INCORRECT but Bare CORRECT: {len(surf_wrong_bare_right)}")

    print("\n=== Where Nowing is wrong but long-context is right (top 5) ===")
    for qid in surf_wrong_lc_right[:5]:
        arms = by_q[qid]
        b = arms.get("bare_llm", {})
        print(f"\n[{qid}] domain={b.get('domain')} qtype={b.get('question_type')}")
        print(f"  GOLD: {b.get('gold')!r}")
        for arm_name in ("bare_llm", "long_context", "nowing"):
            a = arms.get(arm_name, {})
    ...
    print("\n=== Where Long-Context is wrong but Nowing is right (top 5) ===")
