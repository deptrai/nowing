...
* **No Nowing backend or celery required.** Nowing had 0
  reported failures; this script will skip any ``nowing_agentic``
...
from nowing_evals.core.arms import (  # noqa: E402
    ArmRequest,
    ArmResult,
    BareLlmArm,
    NativePdfArm,
)
from nowing_evals.core.parse.freeform_answer import (  # noqa: E402
    extract_freeform_answer,
)
from nowing_evals.core.providers.openrouter_chat import (  # noqa: E402
    OpenRouterChatProvider,
)
from nowing_evals.core.providers.openrouter_pdf import (  # noqa: E402
    OpenRouterPdfProvider,
    PdfEngine,
)
from nowing_evals.suites.multimodal_doc.mmlongbench.grader import grade  # noqa: E402
from nowing_evals.suites.multimodal_doc.parser_compare.prompt import (  # noqa: E402
...
async def _run(args: argparse.Namespace) -> int:
    ...
    # Nowing rows: warn and skip; we don't want to start backend just to
    # defensively retry a 0-failure arm.
    surf_failed = [f for f in failed if f.arm == "nowing_agentic"]
    if surf_failed:
        logger.warning(
            "Skipping %d nowing_agentic failures; this script doesn't drive the backend. "
            "If you want those retried too, start backend + celery and rerun "
            "with --include-nowing.",
            len(surf_failed),
        )
        if not args.include_nowing:
            failed = [f for f in failed if f.arm != "nowing_agentic"]
    else:
        logger.info("No nowing_agentic failures; backend/celery not needed for this retry.")
...
def main() -> None:
    ...
    parser.add_argument(
        "--include-nowing",
        action="store_true",
        help="Also retry nowing_agentic failures (requires backend + celery up). "
        "Default is to skip them since the n=171 run had 0 Nowing failures.",
    )
