"""Query Nowing for the status of every MMLongBench PDF in scope.

Uses the existing Nowing documents client to query
...
async def main() -> None:
    ...
    base = os.environ.get("NOWING_API_BASE", "http://localhost:8000").rstrip("/")
    token = os.environ.get("NOWING_JWT")
    if not token:
        raise SystemExit("NOWING_JWT missing from .env")
