...
from nowing_evals.core.auth import acquire_token, client_with_auth
...
async def test_smoke_against_localhost():
    if "NOWING_API_BASE" not in os.environ:
        pytest.skip("NOWING_API_BASE not set; skipping integration smoke")
    config = load_config()
    ...
    async with client_with_auth(config, bundle) as client:
        response = await client.get(f"{config.nowing_api_base}/api/v1/model-connections/global")
        try:
