...
from nowing_evals.core.auth import (
    CredentialError,
    acquire_token,
    client_with_auth,
)
...
async def test_acquire_token_jwt_mode_short_circuits():
    config = _make_config(nowing_jwt="abc", nowing_refresh_token="ref")
...
async def test_acquire_token_local_mode_posts_desktop_login_json():
    ...
    config = _make_config(nowing_user_email="u@example.com", nowing_user_password="pw")
    bundle = await acquire_token(config)
...
async def test_acquire_token_no_credentials():
    ...
    assert "NOWING_USER_EMAIL" in str(exc.value)
    assert "NOWING_JWT" in str(exc.value)


@pytest.mark.asyncio
@respx.mock
async def test_client_with_auth_refreshes_on_401():
    config = _make_config(nowing_jwt="old", nowing_refresh_token="ref")
    bundle = await acquire_token(config)
