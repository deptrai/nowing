...
| --- | --- |
| `journey.spec.ts` | "I connect Google Drive, choose a file, wait for indexing, and Nowing contains that file's content." |
...
- OAuth state freshness/tamper/malformed validation:
  `nowing_backend/tests/unit/utils/test_oauth_security.py`
- OAuth denied callback and duplicate/reconnection branch:
  `nowing_backend/tests/integration/composio/test_oauth_callback.py`
- Folder listing, selected file config persistence, and auth-expired
  classification:
  `nowing_backend/tests/integration/composio/test_drive_folders_route.py`
...
1. Add fixture data to
   `nowing_backend/tests/e2e/fakes/fixtures/<toolkit>_*.json`.
2. Extend `_Tools.execute()` in
   `nowing_backend/tests/e2e/fakes/composio_module.py` to handle the
