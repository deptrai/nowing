 ...
 * uses Nowing's generic MCP OAuth routes. The E2E backend keeps those routes
 * real and patches Notion's external discovery/DCR/token/MCP tool boundaries
 * (see nowing_backend/tests/e2e/fakes/notion_mcp_module.py).
 */
 ...
 * Jira), so this uses Nowing's generic MCP OAuth routes. The E2E backend
 * keeps those routes real and patches the shared Atlassian
 * discovery/DCR/token/MCP tool boundaries (see
 * nowing_backend/tests/e2e/fakes/jira_module.py). The OAuth code is the
 ...
 *
 * The E2E backend keeps Nowing's generic MCP OAuth routes real and
