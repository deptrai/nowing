...
test.describe("Story 24.3 — Zero Sync Kanban Multi-Context & OCC Conflict", () => {
	test("two team members dragging lead cards on Kanban pipeline sync via Zero and handle OCC 409 collision", async ({
		...
		// 2. Create a test lead in the pipeline via backend API
		...
		try {
			...
			const leadCardA = pageA.locator(`[data-testid="lead-card-${leadId}"]`).or(pageA.getByText(leadTitle)).first();
			const leadCardB = pageB.locator(`[data-testid="lead-card-${leadId}"]`).or(pageB.getByText(leadTitle)).first();

			// 6. User A drags the card to the 'Đang tiếp cận' column
			const columnApproachingA = pageA.locator('[data-testid="kanban-column-approaching"]').or(pageA.getByText("Đang tiếp cận")).first();
			if (await leadCardA.isVisible()) {
				await leadCardA.dragTo(columnApproachingA);
			}

			// 7. AC-1: Zero-cache syncs the card transition to User B in real time without page reload
			const approachingColumnB = pageB.locator('[data-testid="kanban-column-approaching"]').or(pageB.getByText("Đang tiếp cận")).first();
			...
			const conflictToast = pageB.getByText(/xung đột dữ liệu|conflict|phiên bản/i).or(pageB.locator(".toast-error"));
			...
			await leadCardA.click();
			const timelineDrawer = pageA.locator('[data-testid="lead-timeline-drawer"]').or(pageA.getByText("Lịch sử tương tác")).first();
