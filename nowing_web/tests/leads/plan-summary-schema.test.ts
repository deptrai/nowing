/**
 * Story 26.27: Pre-Flight Lead Plan Summary schema & logic tests
 * Run directly with tsx: pnpm exec tsx tests/leads/plan-summary-schema.test.ts
 */
...
function testCampaignPlanResponseSchema() {
	const parsed = campaignPlanResponseSchema.safeParse(mockPlanData);
	assert.ok(parsed.success, "campaignPlanResponseSchema must parse valid payload");
	if (parsed.success) {
...
function testSourcePlanAllocationSchema() {
	const bdsAlloc = mockPlanData.source_allocations[0];
	const parsedBds = sourcePlanAllocationSchema.safeParse(bdsAlloc);
	assert.ok(parsedBds.success, "sourcePlanAllocationSchema must parse high coverage allocation");
	if (parsedBds.success) {
		assert.equal(parsedBds.data.location_coverage_quality, "high");
		assert.equal(parsedBds.data.location_coverage_score, 1.0);
		assert.equal(parsedBds.data.status, "ready");
		assert.equal(parsedBds.data.degraded_reason, null);
	}

	const degradedAlloc = mockPlanData.source_allocations[2];
	const parsedDegraded = sourcePlanAllocationSchema.safeParse(degradedAlloc);
	assert.ok(parsedDegraded.success, "sourcePlanAllocationSchema must parse degraded allocation");
	if (parsedDegraded.success) {
		assert.equal(parsedDegraded.data.status, "degraded");
		assert.ok(parsedDegraded.data.degraded_reason?.includes("degraded"));
	}
}

function testCoverageQualityMapping() {
...
function testCostEstimationFormulas() {
	const maxLeads = 200;
...
function testFallbackWarningParsing() {
...
testCampaignPlanResponseSchema();
testSourcePlanAllocationSchema();
testCoverageQualityMapping();
testCostEstimationFormulas();
testFallbackWarningParsing();
console.log("All Story 26.27 pre-flight plan assertions passed successfully!");
