...
test.describe("Story 26.5: Glass Box Mission Control & Shimmer Influx E2E", () => {
	test(
		"[P0] renders the 4-stage stepper with phase, progress and token velocity",
		async ({ page, request, workspace, apiToken }) => {
			...
			await expect(page.getByTestId("mission-control-stepper")).toContainText(
				"Crawl"
			);
			await expect(page.getByTestId("mission-control-stepper")).toContainText(
				"Reasoning"
	...
	test(
		"[P0] control endpoint response is redacted and never exposes PII",
		async ({ page, request, workspace, apiToken }) => {
			// Intercept the control API and assert the response shape
			await page.route(
				`**/api/v1/workspaces/${workspace.id}/dsh/missions/*/control`,
				async (route) => {
	...
	test(
		"[P0] widget shows 0 tokens/sec and the available cost when token counts are missing",
		async ({ page, request, workspace, apiToken }) => {
			// Mock a control response that has cost but no token counts
			await page.route(
				`**/api/v1/workspaces/${workspace.id}/dsh/missions/*/control`,
				async (route) => {
					await route.fulfill({
			...
			await expect(page.getByTestId("token-velocity-value")).toContainText(
				"0 tokens/sec"
			);
			await expect(page.getByTestId("token-velocity-cost")).toContainText(
				"0.012"
			);
		}
	);
	...
	test(
		"[P0] lead matrix renders 1-3 shimmer rows during ingestion and highlights new rows",
		async ({ page, workspace }) => {
			// Mock a mission in the ingestion phase so the matrix shows shimmer
			await page.route(
				`**/api/v1/workspaces/${workspace.id}/dsh/missions?*`,
				async (route) => {
					await route.fulfill({
						status: 200,
						contentType: "application/json",
	...
	test(
		"[P1] new rows from Zero show the placeholder phone until REST refresh",
		async ({ page, workspace }) => {
			await page.route(
				`**/api/v1/workspaces/${workspace.id}/leads*`,
				async (route) => {
					await route.fulfill({
						status: 200,
						contentType: "application/json",
