...
function buildSeoTitle(model: AnonModel): string {
	...
	return `Chat with ${model.name} Free, No Login | Nowing`;
}
	...
	return [
		{
			...
			answer: `Yes. ${model.name} is available on Nowing without login. No account creation, no email, no password. Just open the page and start chatting with ${model.name} for free.`,
		},
		{
			question: `Is ${model.name} really free on Nowing?`,
			answer: `Yes! You can use ${model.name} completely free without login or sign-up. Nowing gives you 500,000 free tokens to use across any model, including ${model.name}.`,
		},
		{
		...
		{
			question: `What can I do with ${model.name} on Nowing?`,
			answer: `You can ask questions, get explanations, write content, brainstorm ideas, debug code, and more. ${model.name} is a powerful AI assistant available for free without login on Nowing.`,
		},
		{
			question: `How is Nowing different from using ${model.name} directly?`,
			answer: `Nowing gives you free access without login to ${model.name} and many other AI models in one place. Create a free account to unlock document Q&A, team collaboration, and integrations with Slack, Google Drive, Notion, and more.`,
		},
	];
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
	...
	const canonicalUrl = `https://www.nowing.com/free/${model.seo_slug}`;
			...
			images: [
				{
					...
					width: 1200,
					height: 630,
					alt: `${model.name} Free Chat on Nowing`,
				},
			],
		},
		twitter: {
		...
		<>
			...
			<JsonLd
				data={{
					...
					"@type": "WebApplication",
					name: `${model.name} Free Chat Without Login - Nowing`,
					description,
					url: `https://www.nowing.com/free/${model.seo_slug}`,
