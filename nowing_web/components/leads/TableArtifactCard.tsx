...
export const TableArtifactCard: React.FC<TableArtifactCardProps> = ({
	...
	return (
		<div className={cn("my-2.5 not-prose select-none", className)}>
			...
			<div
				className={cn(
					"w-full flex items-center justify-between px-3.5 py-2.5 rounded-lg border transition-all shadow-2xs group",
					isActive
			...
			>
				...
				>
					...
					<div className="min-w-0 flex-1">
						<div className="text-[13px] sm:text-sm font-medium text-foreground truncate group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
							{title}
						</div>
						<div className="flex items-center gap-2 mt-0.5">
							<span className="font-mono text-[10px] font-bold px-1.5 py-0.2 rounded bg-muted text-muted-foreground uppercase tracking-wider">
								{type}
							</span>
							{typeof rowCount === "number" && (
								<span className="text-[11px] text-muted-foreground">• {rowCount} kết quả</span>
							)}
						</div>
					</div>
				</button>
				...
				<div className="flex items-center gap-2 shrink-0 ml-2">
					...
					{isActive && (
						<span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[11px] font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border border-emerald-500/20">
