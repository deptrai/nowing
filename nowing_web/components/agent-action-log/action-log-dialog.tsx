	...
	return (
		<div className="flex flex-1 flex-col items-center justify-center gap-4 px-6 pb-12 text-center">
			...
			<div className="flex max-w-[280px] flex-col gap-1.5">
				<p className="text-sm font-semibold tracking-tight">{t("disabled_title")}</p>
				<p className="text-xs leading-relaxed text-muted-foreground">
					{t("disabled_desc")}{" "}
				...
				<div className="flex min-h-0 flex-1 flex-col overflow-y-auto scrollbar-thin">
					{!actionLogEnabled ? (
						...
						<div className="flex flex-1 flex-col items-center justify-center gap-2 px-6 text-center">
							<p className="text-sm font-medium text-destructive">{t("load_failed")}</p>
						...
						<div className="flex flex-col gap-2 px-4 pb-4">
							...
							{data?.has_more ? (
								<p className="py-2 text-center text-[11px] text-muted-foreground">
									{t("showing", { shown: items.length, total: data.total })}
								</p>
							) : null}
						</div>
					)}
