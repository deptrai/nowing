...
>(function DocumentMentionPicker(
	...
	const nowingDocsQueryParams = useMemo(() => {
	...
	/**
	 * TanStack Query for Nowing documentation.
	 * - Uses AbortSignal for automatic request cancellation
	 * - placeholderData: keepPreviousData prevents UI flicker during refetches
	 */
	const { data: nowingDocs, isLoading: isNowingDocsLoading } = useQuery({
		queryKey: ["nowing-docs-mention", debouncedSearch, isSearchValid],
		queryFn: ({ signal }) =>
			documentsApiService.getNowingDocs({ queryParams: nowingDocsQueryParams }, signal),
		staleTime: 3 * 60 * 1000,
	...
	useEffect(() => {
		if (currentPage === 0) {
			...
			// Nowing docs displayed first in the list
			if (nowingDocs?.items) {
				for (const doc of nowingDocs.items) {
					combinedDocs.push({
						id: doc.id,
						title: doc.title,
	...
	}, [titleSearchResults, nowingDocs, currentPage, filterBySearchTerm]);
	...
	const actualLoading =
		(isTitleSearchLoading || isNowingDocsLoading) &&
		currentPage === 0 &&
		!isSingleCharSearch &&
		accumulatedDocuments.length === 0;
	// Partition documents by type for grouped UI rendering
	const nowingDocsList = useMemo(
		() => actualDocuments.filter((doc) => doc.document_type === "NOWING_DOCS"),
		[actualDocuments]
	);
