...
function buildCurl({ platform, verb, requestBody }: ApiSample): string {
	...
	return [
		`curl -X POST "$NOWING_API_URL/workspaces/$WORKSPACE_ID/scrapers/${platform}/${verb}" \\`,
...
function buildPython({ platform, verb, requestBody }: ApiSample): string {
	...
	return [
		...
		"import requests",
		"",
		"base = os.environ['NOWING_API_URL']",
		"key = os.environ['NOWING_API_KEY']",
		"",
		"resp = requests.post(",
...
function buildJavaScript({ platform, verb, requestBody }: ApiSample): string {
	return [
		...
		"  `${process.env.NOWING_API_URL}/workspaces/${WORKSPACE_ID}/scrapers/" +
			platform +
			"/" +
			verb +
			"`,",
		...
		'    method: "POST",',
		"    headers: {",
		"      Authorization: `Bearer ${process.env.NOWING_API_KEY}`,",
...
function buildGo({ platform, verb, requestBody }: ApiSample): string {
	return [
		...
		'\turl := os.Getenv("NOWING_API_URL") + "/workspaces/" +',
		'\t\tos.Getenv("WORKSPACE_ID") + "/scrapers/' + platform + "/" + verb + '"',
		'\treq, _ := http.NewRequest("POST", url, bytes.NewReader(payload))',
		'\treq.Header.Set("Authorization", "Bearer "+os.Getenv("NOWING_API_KEY"))',
...
function buildPhp({ platform, verb, requestBody }: ApiSample): string {
	return [
		...
		'    CURLOPT_URL => getenv("NOWING_API_URL")',
		'        . "/workspaces/" . getenv("WORKSPACE_ID")',
		'        . "/scrapers/' + platform + "/" + verb + '",',
		"    CURLOPT_POST => true,",
		"    CURLOPT_RETURNTRANSFER => true,",
		"    CURLOPT_HTTPHEADER => [",
		'        "Authorization: Bearer " . getenv("NOWING_API_KEY"),',
		'        "Content-Type: application/json",',
		"    ],",
...
function buildRuby({ platform, verb, requestBody }: ApiSample): string {
	return [
		...
		'require "uri"',
		"",
		"uri = URI(\"#{ENV['NOWING_API_URL']}/workspaces/#{ENV['WORKSPACE_ID']}/scrapers/" +
			platform +
			"/" +
			verb +
			'")',
		"req = Net::HTTP::Post.new(uri)",
		'req["Authorization"] = "Bearer #{ENV[\'NOWING_API_KEY\']}"',
...
function buildJava({ platform, verb, requestBody }: ApiSample): string {
	return [
		...
		'    .uri(URI.create(System.getenv("NOWING_API_URL")',
		'        + "/workspaces/" + System.getenv("WORKSPACE_ID")',
		'        + "/scrapers/' + platform + "/" + verb + '"))',
		'    .header("Authorization", "Bearer " + System.getenv("NOWING_API_KEY"))',
...
function buildCsharp({ platform, verb, requestBody }: ApiSample): string {
	return [
		...
		'var url = Environment.GetEnvironmentVariable("NOWING_API_URL")',
		...
		"};",
		'request.Headers.Add("Authorization", "Bearer " + Environment.GetEnvironmentVariable("NOWING_API_KEY"));',
