		...
		<div className="my-4 max-w-lg overflow-hidden rounded-2xl border bg-muted/30 transition-[box-shadow] duration-300">
			...
			<div className="flex items-start justify-between px-5 pt-5 pb-4 select-none">
				<div>
					...
					{phase === "processing" ? (
						<TextShimmerLoader
							text={hasPanelEdits ? t("confluence_updating_page_with_changes") : t("confluence_updating_page")}
							size="sm"
						/>
					) : phase === "complete" ? (
						...
						<p className="text-xs text-muted-foreground mt-0.5">{t("confluence_update_cancelled")}</p>
					) : (
						<p className="text-xs text-muted-foreground mt-0.5">
					...
					<div className="px-5 py-4 space-y-4 select-none">
						{context?.error ? (
							...
							<>
								...
								{page && (
									<div className="space-y-2">
										<p className="text-xs font-medium text-muted-foreground">{t("confluence_current_page")}</p>
