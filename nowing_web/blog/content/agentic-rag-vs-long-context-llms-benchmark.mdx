...
| `nowing_agentic` | Full agentic RAG pipeline | $10 per 1,000 pages (one-time ingest) | Only the chunks the agent decides to retrieve |
...
If you want to read the implementations, every arm lives in [`nowing_evals/src/nowing_evals/core/arms/`](https://github.com/nowing/Nowing/tree/main/nowing_evals/src/nowing_evals/core/arms) — the [`bare_llm.py`](https://github.com/nowing/Nowing/blob/main/nowing_evals/src/nowing_evals/core/arms/bare_llm.py) arm handles full-context stuffing, [`native_pdf.py`](https://github.com/nowing/Nowing/blob/main/nowing_evals/src/nowing_evals/core/arms/native_pdf.py) handles vision-LLM PDF attachment, and [`nowing.py`](https://github.com/nowing/Nowing/blob/main/nowing_evals/src/nowing_evals/core/arms/nowing.py) drives the agentic retrieval against the Nowing `/api/v1/new_chat` endpoint. The full benchmark suite (prompts, ingest pipeline, runner) lives in [`suites/multimodal_doc/parser_compare/`](https://github.com/nowing/Nowing/tree/main/nowing_evals/src/nowing_evals/suites/multimodal_doc/parser_compare).

We graded answers with a [deterministic, format-aware grader](https://github.com/nowing/Nowing/blob/main/nowing_evals/src/nowing_evals/suites/multimodal_doc/mmlongbench/grader.py) (1% relative tolerance for floats, F1 over normalised tokens for lists). We logged input/output tokens, cost, latency, and any HTTP error per question.
...
<img
  ...
  alt="Bar chart of post-retry accuracy on 171 long-PDF questions: LlamaCloud premium 59.6%, Azure premium 58.5%, Azure basic 54.4%, Nowing agentic RAG 53.2%, Native PDF 52.0%, LlamaCloud basic 50.9%."
  width={2200}
  height={1240}
  style={{ width: '100%', height: 'auto', borderRadius: '12px' }}
/>
...
| 4 | Nowing agentic RAG | 53.2% | 54.3% | 52.8 s | **0** |
...
<img
  ...
  alt="Scatter plot of accuracy versus cost per query for six document-QA approaches; Nowing agentic RAG sits at the cheapest end with competitive accuracy."
  width={2200}
  height={1280}
  style={{ width: '100%', height: 'auto', borderRadius: '12px' }}
/>
...
|---|---:|---:|
| Nowing agentic RAG | **$0.0827** | 53.2% |
...
Of 1,026 total `(arm, question)` cells, 37 returned no answer on the first pass. We then re-ran *only* those 37 with up to 5 attempts of exponential backoff (the [`retry_failed_questions.py`](https://github.com/nowing/Nowing/blob/main/nowing_evals/scripts/retry_failed_questions.py) script in the harness). The results separated **transient** (network/server) failures from **intrinsic** (the approach actually cannot do this) failures:
...
We ran McNemar's exact-binomial test on every pair of arms (15 pairs total) using [`compute_blog_extras.py`](https://github.com/nowing/Nowing/blob/main/nowing_evals/scripts/compute_blog_extras.py). McNemar is the right test for paired classifier comparisons: both arms answered the *same* questions, so we can ask: "of the questions where the two arms disagreed, did one really win more often than chance?"

The result: **only 3 of 15 pairs are distinguishable at α = 0.05**.

The three statistically-significant gaps are all between the *worst* arms (LlamaCloud basic, Native PDF) and the *best* arms (LlamaCloud premium, Azure premium). The most interesting comparison, **Nowing agentic RAG vs the long-context premium arms**, does *not* clear the significance bar. The 6-point gap could plausibly be sample noise.
...
Everything that produced these numbers is open-core. The eval harness is its own package inside the Nowing monorepo:

- [`nowing_evals/`](https://github.com/nowing/Nowing/tree/main/nowing_evals) — the harness root (extensible base classes, providers, cost ledger).
- [`suites/multimodal_doc/parser_compare/`](https://github.com/nowing/Nowing/tree/main/nowing_evals/src/nowing_evals/suites/multimodal_doc/parser_compare) — the benchmark used in this post (prompts, ingest, runner).
- [`core/arms/bare_llm.py`](https://github.com/nowing/Nowing/blob/main/nowing_evals/src/nowing_evals/core/arms/bare_llm.py), [`native_pdf.py`](https://github.com/nowing/Nowing/blob/main/nowing_evals/src/nowing_evals/core/arms/native_pdf.py), [`nowing.py`](https://github.com/nowing/Nowing/blob/main/nowing_evals/src/nowing_evals/core/arms/nowing.py) — the three arm implementations.
- [`mmlongbench/grader.py`](https://github.com/nowing/Nowing/blob/main/nowing_evals/src/nowing_evals/suites/multimodal_doc/mmlongbench/grader.py) — the deterministic format-aware grader.
- [`scripts/retry_failed_questions.py`](https://github.com/nowing/Nowing/blob/main/nowing_evals/scripts/retry_failed_questions.py) — the failed-only retry pass with exponential backoff.
- [`scripts/compute_blog_extras.py`](https://github.com/nowing/Nowing/blob/main/nowing_evals/scripts/compute_blog_extras.py) — the McNemar pairwise tests, latency/token percentiles, and per-PDF heterogeneity.
- [`scripts/compute_post_retry_accuracy.py`](https://github.com/nowing/Nowing/blob/main/nowing_evals/scripts/compute_post_retry_accuracy.py) — merges retry survivors back into the run and recomputes the headline numbers.

A minimal end-to-end run looks like:

```bash
# 1. Clone, install (uv recommended)
git clone https://github.com/nowing/Nowing
cd Nowing/nowing_evals
uv sync --extra dev

# 2. Configure provider keys (Azure DI, LlamaCloud, OpenRouter, Nowing)
cp .env.example .env
$EDITOR .env

# 3. Ingest the first 30 PDFs from MMLongBench-Doc into all parsers
uv run python -m nowing_evals.cli setup multimodal_doc \
  --vision-llm anthropic/claude-sonnet-4.5
uv run python -m nowing_evals.cli ingest multimodal_doc \
  --suite parser_compare --max-docs 30

# 4. Run all six arms × all 171 questions
uv run python -m nowing_evals.cli run multimodal_doc \
...
If you find that the rankings shuffle on your own document set, we want to hear about it. Open an issue on [the Nowing repo](https://github.com/nowing/Nowing/issues) with the run artifacts and we will link your results from this post.

The eval harness is open-core and runs against any OpenRouter model, so you can re-run the same questions on `openai/gpt-4o`, `google/gemini-2.5-pro`, or whichever model you are evaluating for production. Wire your own RAG framework into the [`Arm` base class](https://github.com/nowing/Nowing/blob/main/nowing_evals/src/nowing_evals/core/arms/base.py) (LangChain, LlamaIndex, Haystack, your own stack) and you can drop it into the same comparison without changing the rest of the pipeline.

If you want a hosted way to try agentic RAG on your own PDFs without writing the harness yourself, [Nowing](/free) is one option (it is the same agentic stack that powered the `nowing_agentic` arm above).
