...
> - **Want Gemini?** Google requires a Google account on `gemini.google.com`. The closest no-signup path is an open-core aggregator like our own [Nowing /free](/free), which lists Gemini among its model options without a Google sign-in.
> - **Want all of them in one place?** Our open-core [Nowing /free](/free) lets you pick from ChatGPT, Claude, Gemini, DeepSeek, Mistral, Llama, and a rotating list of other models with no account, and you get 500,000 free tokens to spend across any of them. [Duck.ai](https://duck.ai) and [Brave Leo](https://brave.com/leo/) are strong privacy-first alternatives.
...
You can also use **[Nowing /free](/free)** (full disclosure: this is our own open-core aggregator) which lists ChatGPT among its no-signup models. You get 500,000 free tokens to spend across any model on the page, which is meaningfully more than what guest mode gives you before the signup wall. The source code is on [GitHub](https://github.com/nowing/Nowing) so the privacy and quota behavior is auditable, not just promised.
...
**Open-core wrappers** are a half-step better than the closed ones, and worth calling out as a separate category. Our own [Nowing /free](/free) is in this bucket: the source code that handles your prompt is on [GitHub](https://github.com/nowing/Nowing), so the claims about anonymous sessions, no persistent identity, and no prompt retention are auditable rather than promised. That doesn't make it equivalent to Brave Leo or Duck.ai (the model provider behind /free still receives the prompt content), but it does mean you can verify the wrapper layer doesn't add its own logging on top. If you're going to use a wrapper anyway, prefer one whose code you can read.
...
- For Claude, [Duck.ai](https://duck.ai) (Haiku, anonymised) or an aggregator like [Nowing /free](/free) which lists Claude among its model options.
- For multi-model comparison in one tab, [Nowing /free](/free) or [Duck.ai](https://duck.ai).
...
If you want a graphical interface instead of a terminal, install [LM Studio](https://lmstudio.ai). For a hosted-but-self-controlled experience, the open-core Nowing stack on the [GitHub repo](https://github.com/nowing/Nowing) gives you the same chat UI with the same model options, running on your own servers.
