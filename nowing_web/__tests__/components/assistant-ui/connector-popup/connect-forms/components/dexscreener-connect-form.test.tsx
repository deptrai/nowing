...
describe("DexScreenerConnectForm", () => {
	...
	describe("Initial Rendering", () => {
		it("should render the form with all required fields", () => {
			render(
				<DexScreenerConnectForm onSubmit={mockOnSubmit} onBack={mockOnBack} isSubmitting={false} />
			);
	...
	describe("Token Management", () => {
		...
		it("should disable Add Token button when maximum tokens (50) are reached", () => {
			render(
				<DexScreenerConnectForm onSubmit={mockOnSubmit} onBack={mockOnBack} isSubmitting={false} />
			);
			...
			// Use synchronous fireEvent to avoid per-click async overhead (49 iterations)
			for (let i = 0; i < 49; i++) {
				fireEvent.click(addTokenButton);
			}

			// State is synchronous after fireEvent — assert directly
