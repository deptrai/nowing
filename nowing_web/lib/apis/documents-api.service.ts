import {
	...
	getNowingDocsByChunkResponse,
	getNowingDocsRequest,
	getNowingDocsResponse,
...
class DocumentsApiService {
	...
	/**
	 * Get Nowing documentation by chunk ID
	 * Used for resolving [citation:doc-XXX] citations
	 */
	getNowingDocByChunk = async (chunkId: number) => {
		return baseApiService.get(
			`/api/v1/nowing-docs/by-chunk/${chunkId}`,
			getNowingDocsByChunkResponse
		);
	};
	 ...
	 * @param signal - Optional AbortSignal for request cancellation
	 */
	getNowingDocs = async (request: GetNowingDocsRequest, signal?: AbortSignal) => {
		const parsedRequest = getNowingDocsRequest.safeParse(request);
		...
		const url = `/api/v1/nowing-docs?${queryParams}`;

		return baseApiService.get(url, getNowingDocsResponse, { signal });
	};
