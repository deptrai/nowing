...
export function classifyChatError(input: RawChatErrorInput): NormalizedChatError {
	...
	if (errorCode === "PREMIUM_QUOTA_EXHAUSTED") {
		return {
			...
			isExpected: true,
			userMessage: "chat.error_buy_tokens",
	...
	if (errorCode === "TURN_CANCELLING") {
		return {
			...
			telemetryEvent: "chat_blocked",
			isExpected: true,
			userMessage: "chat.error_turn_stopping",
			rawMessage,
	...
	if (errorCode === "SEND_FAILED_PRE_ACCEPT") {
		return {
			...
			telemetryEvent: "chat_blocked",
			isExpected: true,
			userMessage: "chat.error_send_failed",
			rawMessage,
	...
	if (errorCode === "AUTH_EXPIRED" || errorCode === "UNAUTHORIZED") {
		return {
			...
			telemetryEvent: "chat_error",
			isExpected: true,
			userMessage: "chat.error_auth_expired",
			rawMessage,
	...
	if (errorCode === "MODEL_PROVIDER_UNAVAILABLE") {
		return {
			...
			isExpected: true,
			userMessage:
				"chat.error_provider_unavailable",
			rawMessage,
	...
	if (errorCode === "NETWORK_ERROR") {
		return {
			...
			telemetryEvent: "chat_error",
			isExpected: true,
			userMessage: "chat.error_network",
			rawMessage,
	...
	if (errorCode === "STREAM_PARSE_ERROR") {
		return {
			...
			telemetryEvent: "chat_error",
			isExpected: false,
			userMessage: "chat.error_stream_parse",
			rawMessage,
	...
	if (errorCode === "TOOL_EXECUTION_ERROR") {
		return {
			...
			telemetryEvent: "chat_error",
			isExpected: false,
			userMessage: "chat.error_tool_execution",
			rawMessage,
	...
	if (errorCode === "PERSIST_MESSAGE_FAILED") {
		return {
			...
			telemetryEvent: "chat_error",
			isExpected: false,
			userMessage: "chat.error_persist_failed",
			rawMessage,
