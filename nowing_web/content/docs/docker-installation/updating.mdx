...
Nowing containers are labeled for Watchtower, so `--label-enable` limits updates to the Nowing services.
...
docker run --rm -v /var/run/docker.sock:/var/run/docker.sock \
  ...
  --label-filter "com.docker.compose.project=nowing"
```
...
It detects the legacy `nowing-data` volume, stops the old container, dumps your database with a temporary PostgreSQL 14 container, recovers your `SECRET_KEY`, restores everything into PostgreSQL 17, and starts the new stack. Your original volume is **never deleted** — the dump is also saved to `./nowing_migration_backup.sql` as a checkpoint, so if anything fails mid-way you can simply re-run the script.

After it completes:

1. Open Nowing and confirm your data is intact.
2. Remove the old volume (irreversible): `docker volume rm nowing-data`
3. Delete the dump file once you no longer need it: `rm ./nowing_migration_backup.sql`
