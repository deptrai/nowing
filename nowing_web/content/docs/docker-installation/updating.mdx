...
<Callout type="warn">
If you were previously using `docker-compose.quickstart.yml` (the legacy all-in-one `nowing` container), your data lives in a `nowing-data` volume and requires a **one-time migration** before switching to the current setup. PostgreSQL has been upgraded from version 14 to 17, so a simple volume swap will not work.
