...
description: Run Nowing with Docker in minutes
---

Docker is the recommended way to run Nowing. Everything — database, backend, frontend, background workers, real-time sync, and a reverse proxy — comes pre-configured.
...
The script creates a `./nowing/` directory with the compose files and a `.env`, generates a secret key, starts all services, and waits until they're healthy. It also:
...
- **Nowing**: [http://localhost:3929](http://localhost:3929)
...
All configuration lives in a single `.env` file — `nowing/.env` if you used the install script, `docker/.env` if you cloned manually. The bundled `.env.example` documents every option inline with comments and working defaults; edit the values you need and restart:
...
Use `NOWING_VARIANT=cuda126` for older NVIDIA driver stacks. On Windows, use `;` instead of `:` in `COMPOSE_FILE`. Then apply:
...
```dotenv
NOWING_SITE_ADDRESS=surf.example.com
LISTEN_HTTP_PORT=80
LISTEN_HTTPS_PORT=443
CERT_EMAIL=you@example.com
NOWING_PUBLIC_URL=https://surf.example.com
```
...
To update Nowing, see [Updating](/docs/docker-installation/updating).

## Building from Source (Contributors)

If you're contributing to Nowing and want to build the images from your local checkout, use the dev compose file. Unlike the production setup above, there's no bundled Caddy proxy — each service publishes its own port directly, and you need **three** separate `.env` files instead of one.
...
```bash
git clone https://github.com/<your-fork>/Nowing.git
cd Nowing
cp docker/.env.example docker/.env
cp nowing_backend/.env.example nowing_backend/.env
cp nowing_web/.env.example nowing_web/.env
```
...
- **`SECRET_KEY`** in `nowing_backend/.env` — required, generate with `openssl rand -base64 32`. Note this is separate from `SECRET_KEY` in `docker/.env`; the dev compose file does not forward one to the other.
...
- **Real-time updates not working**: Open DevTools → Console and check for WebSocket errors. The expected URL is `${NOWING_PUBLIC_URL}/zero`.
...
```bash
docker compose down
docker volume rm nowing-zero-cache   # wipe the half-built replica
