...
description: Import your OneDrive files into the Nowing knowledge base
---
...
<Callout type="info">
This setup is only needed on **self-hosted** deployments. On Nowing Cloud, just use **Import → OneDrive**.
</Callout>
...
<Callout type="info">
`http://localhost:3929` is the default public URL of a Docker install. If you run Nowing [manually](/docs/manual-installation), use your backend URL (`http://localhost:8000`); in production, use your public domain.
</Callout>
...
3. Enter a description (e.g., `Nowing`) and pick an expiration period
4. Click **"Add"**
...
## Step 5: Add the Credentials to Nowing
...
Restart Nowing, open **Documents → Import → OneDrive**, and authorize:
