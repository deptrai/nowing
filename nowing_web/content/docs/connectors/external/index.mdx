...
External connectors link Nowing to third-party services. Open the **Connectors** dialog in your workspace, pick a service, and connect — via OAuth, an API token, a webhook, or a companion plugin, depending on the service. You can connect multiple accounts of the same service.
...
These work out of the box on every deployment — Nowing connects to the service's hosted MCP server and the service issues credentials automatically. Just click **Connect**:
...
<Callout type="info">
On Nowing Cloud these are already configured — just click **Connect**. The setup guides above only apply to self-hosted deployments.
</Callout>
...
<Cards>
  ...
  <Card
    title="Obsidian"
    description="Sync your vault with the Nowing Obsidian plugin"
