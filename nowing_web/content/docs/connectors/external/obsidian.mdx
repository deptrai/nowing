---
title: Obsidian
description: Sync your Obsidian vault with the Nowing plugin
---

# Obsidian Plugin Setup Guide

Nowing integrates with Obsidian through the Nowing Obsidian plugin.

## How it works

The plugin runs inside your Obsidian app and pushes note updates to Nowing over HTTPS.
...
1. Open **Connectors** in Nowing and choose **Obsidian**.
2. Install the plugin (recommended via BRAT) using the steps below.
3. In Obsidian, open **Settings → Nowing**.  
4. Paste your Nowing API token from the user settings section.
5. Paste your Server URL in the plugin setting: either your Nowing main domain (if `/api/v1` rewrites are enabled) or your direct backend URL.
6. Choose the Workspace in the plugin, then the first sync should run automatically.
7. Confirm the connector appears as **Obsidian - &lt;vault&gt;** in Nowing.
...
3. Paste the repository: `https://github.com/nowing/Nowing/`.
4. Select the latest plugin version, then click "Add plugin".
5. Open **Settings → Nowing** to finish setup.
...
1. Delete the old legacy Obsidian connector from Nowing.
2. Install and configure the Nowing Obsidian plugin using the quick start above.
...
- Regenerate and paste a fresh API token from Nowing.
...
- Avoid using localhost unless Nowing and Obsidian run on the same machine.
