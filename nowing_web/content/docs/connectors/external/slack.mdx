---
title: Slack
description: Let the Nowing agent search and read your Slack workspace
---

The Slack connector connects Nowing to Slack's hosted MCP server, giving the agent live tools to search channels and read threads on your behalf. Nothing is indexed in the background — the agent queries Slack in real time when you ask.
...
3. Enter your Nowing callback URL:
...
<Callout type="info">
`http://localhost:3929` is the default public URL of a Docker install. If you run Nowing [manually](/docs/manual-installation), use your backend URL (`http://localhost:8000`); in production, use your public domain. Note that Slack requires HTTPS redirect URLs for production apps.
</Callout>
...
## Step 7: Add the Credentials to Nowing
...
Restart Nowing, open the **Connectors** dialog, and click **Connect** on Slack:
