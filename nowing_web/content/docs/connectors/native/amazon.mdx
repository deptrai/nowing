...
```json
{
  "urls": [
    ...
    "https://www.amazon.fr/s?k=clavier+mécanique",
...
For `search_terms`, set `domain` to the marketplace you want to search. For direct URLs, the scraper infers the marketplace from each URL. Each returned item carries a `marketplace` field (e.g. `de`, `co.uk`, `fr`) so consumers know which region the data came from.
