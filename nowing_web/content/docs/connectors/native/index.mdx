...
description: Nowing's built-in scraper APIs for Reddit, YouTube, Instagram, TikTok, Amazon, Google Maps, Google Search, and the web
---

import { Card, Cards } from 'fumadocs-ui/components/card';

Native connectors are Nowing's own scraper APIs — built into the platform, no third-party account or OAuth app required. They pull structured, public data from the platforms below and return clean JSON.
...
<Cards>
  ...
  <Card
    title="Indeed"
    description="Job listings and job details from Indeed"
  ...
  <Card
    ...
    href="/docs/connectors/native/vn_jobs"
  />
</Cards>
...
1. **In chat** — the AI agent uses these scrapers as tools automatically. Ask "what is r/selfhosted saying about Nowing?" and the agent runs the Reddit scraper for you.
...
Scrapers make real requests to the target platforms, and heavy use from a single server IP will eventually get rate-limited or blocked. If you self-host, we recommend routing scraper traffic through a proxy — any HTTP proxy or rotating residential/datacenter gateway you already use works; Nowing doesn't require a specific vendor.
...
# Or a comma-separated pool that Nowing rotates through per request.
...
<Callout type="info">
On Nowing Cloud, proxying is already handled — nothing to configure.
</Callout>
