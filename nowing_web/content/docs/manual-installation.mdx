...
description: Set up Nowing from source, component by component
icon: Wrench
---

This guide sets up Nowing without Docker (except for zero-cache, which is simplest to run as a container). Choose this path if you want to contribute to Nowing or need full control over each component. If you just want to run Nowing, the [Docker installation](/docs/docker-installation) is much faster.
...
```bash
git clone https://github.com/nowing/Nowing.git
cd Nowing
```

## Decisions to Make Up Front

**Authentication.** Nowing supports local email/password auth (the default, no setup needed) or Google OAuth login. For Google login you need an OAuth client from the [Google Cloud Console](https://console.cloud.google.com/apis/credentials) — the [Google connectors guide](/docs/connectors/external/google) walks through creating one.

**Document parsing (ETL).** Nowing converts uploaded files with one of three services:
...
Nowing uses [Rocicorp Zero](https://zero.rocicorp.dev/) for real-time updates (notifications, document status, chat comments, indexing progress). Zero replicates data from PostgreSQL via **logical replication**, which requires a one-time PostgreSQL configuration change.
...
```bash
psql -U postgres -d nowing -c "SHOW wal_level;"
...
```sql
ALTER USER nowing WITH REPLICATION;
GRANT CREATE ON DATABASE nowing TO nowing;
```
...
```bash
psql -U postgres -d nowing -c "SELECT pubname FROM pg_publication;"
...
```powershell
cd nowing_backend
uv run celery -A celery_worker.celery_app worker --loglevel=info --concurrency=1 --pool=solo --queues="nowing,nowing.connectors,nowing.gateway"
```
...
```bash
docker run -d --name nowing-zero-cache \
  -p 4848:4848 \
  --add-host=host.docker.internal:host-gateway \
  -e ZERO_UPSTREAM_DB="postgresql://postgres:postgres@host.docker.internal:5432/nowing?sslmode=disable" \
  -e ZERO_CVR_DB="postgresql://postgres:postgres@host.docker.internal:5432/nowing?sslmode=disable" \
  -e ZERO_CHANGE_DB="postgresql://postgres:postgres@host.docker.internal:5432/nowing?sslmode=disable" \
...
docker logs -f nowing-zero-cache
```
...
The deps-only stack exposes zero-cache on port `4848`. Point the frontend at it by setting the zero-cache URL in `nowing_web/.env` (see the comments in `nowing_web/.env.example`).
...
The Nowing browser extension saves any webpage — including those behind authentication — straight into your knowledge base.
...
Load the built extension in your browser's developer mode and configure it with your Nowing API key. See the [Plasmo build docs](https://docs.plasmo.com/framework/workflows/build#with-a-specific-target) for details.
...
- **Real-time updates not working / stale UI**: Verify zero-cache is running (`curl http://localhost:4848/keepalive` returns 200). Open browser DevTools → Console and look for WebSocket errors. Confirm the zero-cache URL in `nowing_web/.env` matches the running zero-cache address.
- **Zero-cache stuck on `Unknown or invalid publications. Specified: [zero_publication]`**: You skipped `uv run alembic upgrade head`. Run it from `nowing_backend/`, then `docker restart nowing-zero-cache`.
- **Zero-cache crashes with `_zero.tableMetadata` errors**: A previous run left a half-built SQLite replica behind. Start fresh: `docker rm -f nowing-zero-cache && docker volume rm nowing-zero-cache`, then re-run the command from [Zero-Cache Setup](#zero-cache-setup).
