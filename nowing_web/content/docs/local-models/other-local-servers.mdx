...
Use this when Nowing is running from Docker and the model server is running on your computer.
...
Use this when Nowing and the model server both run directly on the same computer.
...
If you enter the URL without `/v1`, Nowing adds `/v1` for requests.
...
Load or serve a model, then refresh model discovery in Nowing.
