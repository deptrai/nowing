---
title: Ollama
description: Connect Ollama to Nowing
---
...
Choose the URL from where the Nowing backend runs.

### Nowing Runs in Docker

Use this when Nowing is running from Docker and Ollama is running on your computer.
...
<Callout type="info">
This is the default in Nowing.
</Callout>

### Ollama Runs in Docker

Use this only when Ollama is a service in the same Compose stack as Nowing.

```text
http://ollama:11434
```

### Nowing Runs Without Docker

Use this when Nowing and Ollama both run directly on the same computer.
...
Do not add `/v1` to the URL. Nowing uses Ollama native routes such as `/api/version` and `/api/tags`.
...
From the Nowing backend container:
...
Then refresh model discovery in Nowing.
