...
The Slack messaging channel lets users mention the Nowing bot in Slack and
chat with the Nowing backend agent from a Slack thread.
...
1. Slack sends an `app_mention` event to Nowing.
2. Nowing verifies the Slack signature and stores the event in the gateway inbox.
3. Nowing resolves the Slack user binding to a Nowing user and workspace.
