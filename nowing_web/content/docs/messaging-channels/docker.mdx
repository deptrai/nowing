...
`docker/.env` file. You do not need to edit `nowing_backend/.env.example`.
...
For localhost-only testing, the defaults are enough for the Nowing UI, but
public webhooks from Telegram, WhatsApp, and Slack require a public HTTPS
Nowing URL. Use your deployed domain or a tunnel such as Cloudflare Tunnel
or ngrok.
