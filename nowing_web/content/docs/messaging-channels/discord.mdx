...
The Discord messaging channel lets users mention the Nowing bot in Discord
and chat with the Nowing backend agent from a Discord channel.
...
- **Message Content Intent** so Nowing can read text after a bot mention.
...
2. Nowing stores the event in the durable gateway inbox.
3. Nowing resolves the Discord user binding to a Nowing user and workspace.
4. Nowing runs the backend agent with that user's permissions.
