---
title: Observability
description: Configure backend traces and metrics for Nowing
icon: Radar
---

Nowing uses OpenTelemetry for backend traces and metrics. Application logs
...
`nowing_backend/.env`. Add these values there:
...
When deploying `nowing_backend/Dockerfile` directly instead of production
compose, use the same split: Nowing containers export to a collector, and
...
Nowing keeps project-specific spans behind `app.observability.otel`:
...
metrics. Nowing adds these project metrics from `app.observability.metrics`:
...
Nowing intentionally does not create an OpenTelemetry `LoggerProvider`,
