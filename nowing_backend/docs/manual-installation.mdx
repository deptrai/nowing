...
description: Setting up Nowing manually for customized deployments
icon: Wrench
---

# Manual Installation (Preferred)

This guide provides step-by-step instructions for setting up Nowing without Docker. This approach gives you more control over the installation process and allows for customization of the environment.
...
The backend is the core of Nowing. Follow these steps to set it up:
...
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| DATABASE_URL               | PostgreSQL connection string (e.g., `postgresql+asyncpg://postgres:postgres@localhost:5432/nowing`)                                                                                    |
...
| LANGSMITH_PROJECT | LangSmith project name (e.g., `nowing`) |
...
```bash
# Make sure you're in the nowing_backend directory
cd nowing_backend
...
The Nowing browser extension allows you to save any webpage, including those protected behind authentication.
...
| ENV VARIABLE              | DESCRIPTION                                           |
| ------------------------- | ----------------------------------------------------- |
| PLASMO_PUBLIC_BACKEND_URL | Nowing Backend URL (e.g., `http://127.0.0.1:8000`) |
...
Load the extension in your browser's developer mode and configure it with your Nowing API key.
...
Now that you have Nowing running locally, you can explore its features:
