...
This guide walks you through connecting your Circleback meetings to Nowing for meeting search and AI-powered insights.

## How it works

The Circleback connector uses a **webhook-based integration**. Unlike other connectors that pull data from APIs, Circleback automatically pushes meeting data to Nowing when meetings are processed.

- Meetings are sent to Nowing immediately after Circleback processes them
...
<Callout type="info">
    Meeting recordings are not stored in Nowing, but a link to the recording (valid for 24 hours from Circleback) is included in the indexed document.
</Callout>
...
### Step 1: Create the Connector in Nowing
...
```
https://your-nowing-url/api/v1/webhooks/circleback/{search_space_id}
```
...
6. Enter your Nowing webhook URL
...
<Callout type="warn">
    Make sure your Nowing backend is accessible from the internet for Circleback to send webhooks. If self-hosting, you may need to configure a reverse proxy or use a tunneling service.
</Callout>
...
<Callout type="info">
    Unlike other connectors, Circleback doesn't support periodic indexing since it's webhook-based. Meetings are automatically pushed to Nowing when processed by Circleback.
</Callout>
...
Once configured, new meetings will automatically appear in Nowing after Circleback processes them. To verify:
...
3. Check your Nowing search space for the new meeting document
...
- Check that your Nowing backend is accessible from the internet
- Look for errors in your Nowing backend logs
