...
``PermissionMiddleware`` both used the Nowing-specific
...
async def test_parallel_self_gated_and_middleware_gated_route_and_resume_cleanly():
    ...
    # the unification. If either one regresses to the legacy Nowing shape
    ...
    config["configurable"]["nowing_resume_value"] = by_tool_call_id
