...
class TestConsumeNowingResume:
    def test_pops_only_entry_matching_runtime_tool_call_id(self):
        ...
        assert consume_nowing_resume(runtime) == {"decisions": ["approve"]}
    ...
    def test_popping_one_entry_leaves_siblings_untouched(self):
        ...
        consume_nowing_resume(runtime_a)

        assert configurable["nowing_resume_value"] == {
            "tcid-B": {"decisions": ["reject"]}
        }
    ...
    def test_returns_none_when_no_entry_for_this_tool_call(self):
        runtime = _runtime_with_config(
            {
                "configurable": {
                    "nowing_resume_value": {"tcid-other": {"decisions": []}}
                }
            },
            tool_call_id="tcid-A",
        )
    ...
    def test_drops_empty_dict_after_last_entry_consumed(self):
        ...
        consume_nowing_resume(runtime)

        assert "nowing_resume_value" not in configurable


class TestHasNowingResume:
    def test_true_when_entry_for_this_tool_call_present(self):
        ...
        assert has_nowing_resume(runtime) is True
