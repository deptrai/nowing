...
NOWING_DEFAULTS = Ruleset(
    rules=[Rule(permission="*", pattern="*", action="allow")],
    origin="nowing_defaults",
)
...
class TestDesktopSafetyRulesGateDestructiveOps:
    ...
    def test_destructive_op_resolves_to_ask(self, tool_name: str) -> None:
        # nowing_defaults says "allow */*"; desktop_safety must override
        # because it's layered later (last-match-wins).
        action = _action_for(tool_name, NOWING_DEFAULTS, DESKTOP_SAFETY_RULESET)
        assert action == "ask", (
...
class TestDesktopSafetyOverridesAllowDefault:
    def test_layer_order_last_match_wins(self) -> None:
        # If desktop_safety is layered BEFORE nowing_defaults, the allow
        ...
        action = _action_for("rm", DESKTOP_SAFETY_RULESET, NOWING_DEFAULTS)
        # Layered "wrong way" — the broad allow now wins.
        assert action == "allow"

        # Correct order: defaults < desktop_safety -> ask wins.
        action = _action_for("rm", NOWING_DEFAULTS, DESKTOP_SAFETY_RULESET)
        assert action == "ask"
