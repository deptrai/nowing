...
class TestTelegramMediaStreamingTask:
    ...
    async def test_stream_media_single_put_under_5mb(self) -> None:
        ...
        with (
            patch(
                "app.tasks.telegram_media_tasks.get_s3_client",
                return_value=mock_s3_client,
            ),
            patch(
                "app.tasks.telegram_media_tasks.get_telegram_media_stream",
                side_effect=fake_stream,
            ),
            patch(
                "app.tasks.telegram_media_tasks.update_media_record",
    ...
    async def test_stream_media_multipart_upload_over_5mb(self) -> None:
        ...
        mock_s3_client.complete_multipart_upload = AsyncMock(
            return_value={"Location": "https://s3.nowing.net/media.mp4"}
        )
    ...
    async def test_handles_s3_failure_gracefully(self) -> None:
        ...
        with (
            ...
            mock_db.assert_awaited_with(
                media_id=5680, status="failed", error="S3 Connection Timeout"
            )
