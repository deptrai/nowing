...
``nowing_web/lib/chat/streaming-state.ts``). Every test asserts both
...
class TestToolHeavyTurn:
    def test_full_tool_lifecycle_produces_complete_tool_call_part(self):
        ...
        b.on_tool_input_delta("call_run123", '"nowing"}')
        ...
        assert tool_part["args"] == {"query": "nowing"}
        ...
        assert tool_part["argsText"] == json.dumps(
            {"query": "nowing"}, indent=2, ensure_ascii=False
        )
