"""Unit tests for EtlPipelineService — plaintext, CSV/TSV/HTML, audio, docling, unstructured, llamacloud basic."""
...
@pytest.mark.parametrize(
    "filename,content,expected_markdown",
    [
        ("hello.txt", "Hello, world!", "Hello, world!"),
        ("readme.md", "# Title\n\nBody text.", "# Title\n\nBody text."),
        ("notes.markdown", "Some notes.", "Some notes."),
        ("script.py", "print('hello')", "print('hello')"),
        ("app.js", "console.log('hi');", "console.log('hi');"),
    ],
    ids=["txt", "md", "markdown", "py", "js"],
)
async def test_extract_plaintext_file(tmp_path, filename, content, expected_markdown):
    """Plaintext-family files (.txt/.md/.markdown/.py/.js) are read as-is."""
    f = tmp_path / filename
    ...
    assert result.markdown_content == expected_markdown
