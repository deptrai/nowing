"""Tests for the Nowing OpenTelemetry shim."""
...
def _reset_otel_state(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("NOWING_DISABLE_OTEL", "true")
...
def test_enabled_when_endpoint_configured(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("NOWING_DISABLE_OTEL", raising=False)
...
class TestBootstrapConfig:
    ...
    def test_resource_defaults_include_service_metadata(
        ...
        monkeypatch.setenv("NOWING_ENV", "test")
    ...
    def test_deployment_environment_uses_nowing_env_only(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.delenv("NOWING_ENV", raising=False)

        assert bootstrap._deployment_environment() == "dev"

        monkeypatch.setenv("NOWING_ENV", "production")
