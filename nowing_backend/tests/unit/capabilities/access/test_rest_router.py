...
def _register_nowing_handler(app: FastAPI) -> None:
    """Minimal stand-in for the app's global NowingError handler."""
    from starlette.responses import JSONResponse

    from app.exceptions import NowingError

    async def _handler(_request, exc: NowingError):
        return JSONResponse(
            status_code=exc.status_code,
            content={"code": exc.code, "message": exc.message},
        )

    app.add_exception_handler(NowingError, _handler)


@pytest.mark.asyncio
async def test_executor_fault_becomes_502(monkeypatch):
    """Any non-Nowing executor error is surfaced as a clean 502, not a 500."""
...
@pytest.mark.asyncio
async def test_nowing_error_passes_through(monkeypatch):
