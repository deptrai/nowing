...
@pytest.mark.skip(reason="red-phase: default sources should be all 3")
...
@pytest.mark.skip(reason="red-phase: only listed sources should be called")
async def test_fan_out_only_listed_sources(monkeypatch):
    ...
    monkeypatch.setattr("app.services.jobs_aggregator.orchestrator._call_source", fake_call_source)
    ctx = CapabilityContext(session=SimpleNamespace(), workspace_id=1)
    await aggregate_jobs(VnJobAggregateInput(keyword="dev", sources=["itviec", "topcv"]), ctx)
    assert set(called) == {"itviec", "topcv"}
    assert "vietnamworks" not in called


@pytest.mark.skip(reason="red-phase: max_items_per_source should be passed to source")
async def test_fan_out_max_items_per_source_passed(monkeypatch):
    ...
    monkeypatch.setattr("app.services.jobs_aggregator.orchestrator._call_source", fake_call_source)
    ctx = CapabilityContext(session=SimpleNamespace(), workspace_id=1)
    await aggregate_jobs(VnJobAggregateInput(keyword="dev", max_items_per_source=50), ctx)
...
@pytest.mark.skip(reason="red-phase: _call_source raising KeyError → degraded")
async def test_fan_out_source_key_error_degraded(monkeypatch):
    ...
    monkeypatch.setattr("app.services.jobs_aggregator.orchestrator._call_source", fake_call_source)
    ctx = CapabilityContext(session=SimpleNamespace(), workspace_id=1)
    result = await aggregate_jobs(VnJobAggregateInput(keyword="dev", sources=["itviec"]), ctx)
    assert result.degraded is True
    assert "itviec" in result.degraded_source_ids


@pytest.mark.skip(reason="red-phase: _call_source raising Exception → degraded")
...
@pytest.mark.skip(reason="red-phase: _call_source returning None → empty source")
...
@pytest.mark.skip(reason="red-phase: max_items_per_source=0 → no items")
async def test_fan_out_max_items_zero(monkeypatch):
    ...
    monkeypatch.setattr("app.services.jobs_aggregator.orchestrator._call_source", fake_call_source)
    ctx = CapabilityContext(session=SimpleNamespace(), workspace_id=1)
    result = await aggregate_jobs(VnJobAggregateInput(keyword="dev", max_items_per_source=0), ctx)
    assert len(result.items) == 0
    for payload in captured:
        assert payload.get("max_items") == 0


@pytest.mark.skip(reason="red-phase: empty sources list → default or empty")
...
@pytest.mark.skip(reason="red-phase: cost_micros sum across sources")
async def test_fan_out_cost_micros_sum(monkeypatch):
    ...
    async def fake_call_source(source: str, payload: dict[str, Any], ctx: CapabilityContext) -> dict[str, Any]:
        if source == "vietnamworks":
            return {"items": [{"id": "1", "title": "Dev", "company": "Co"}], "cost_micros": 3500, "degraded": False}
        if source == "itviec":
            return {"items": [{"id": "2", "title": "Dev", "company": "Co"}], "cost_micros": 2000, "degraded": False}
        return {"items": [], "degraded": True, "degradation_reason": "topcv: failed"}

    monkeypatch.setattr("app.services.jobs_aggregator.orchestrator._call_source", fake_call_source)
    ctx = CapabilityContext(session=SimpleNamespace(), workspace_id=1)
    result = await aggregate_jobs(VnJobAggregateInput(keyword="dev", sources=["vietnamworks", "itviec", "topcv"]), ctx)
...
@pytest.mark.skip(reason="red-phase: degraded_source_ids field not on output")
async def test_degradation_source_ids_set(monkeypatch):
    """should return degraded_source_ids listing failed source names."""

    async def fake_call_source(source: str, payload: dict[str, Any], ctx: CapabilityContext) -> dict[str, Any]:
        if source == "topcv":
            return {"items": [], "degraded": True, "degradation_reason": "topcv: tos_pending"}
        return {"items": [{"id": "1", "title": "Dev", "company": "Co"}], "degraded": False}

    monkeypatch.setattr("app.services.jobs_aggregator.orchestrator._call_source", fake_call_source)
    ctx = CapabilityContext(session=SimpleNamespace(), workspace_id=1)
    result = await aggregate_jobs(VnJobAggregateInput(keyword="dev", sources=["vietnamworks", "topcv"]), ctx)
    assert result.degraded is True
    assert "topcv" in result.degraded_source_ids
    assert "vietnamworks" not in result.degraded_source_ids


@pytest.mark.skip(reason="red-phase: degradation_reasons should be canonical enum")
...
@pytest.mark.skip(reason="red-phase: items from successful sources still returned")
async def test_degradation_successful_sources_items_returned(monkeypatch):
    ...
    async def fake_call_source(source: str, payload: dict[str, Any], ctx: CapabilityContext) -> dict[str, Any]:
        if source == "vietnamworks":
            return {"items": [{"id": "1", "title": "Dev", "company": "Co"}], "degraded": False}
        return {"items": [], "degraded": True, "degradation_reason": f"{source}: failed"}

    monkeypatch.setattr("app.services.jobs_aggregator.orchestrator._call_source", fake_call_source)
    ctx = CapabilityContext(session=SimpleNamespace(), workspace_id=1)
    result = await aggregate_jobs(VnJobAggregateInput(keyword="dev", sources=["vietnamworks", "topcv", "itviec"]), ctx)
    assert result.degraded is True
    assert len(result.items) == 1
...
@pytest.mark.skip(reason="red-phase: tos_pending → SOURCE_FAILED mapping")
...
@pytest.mark.skip(reason="red-phase: capability_not_found → SOURCE_FAILED mapping")
...
@pytest.mark.skip(reason="red-phase: 429/rate_limit → RATE_LIMIT mapping")
...
@pytest.mark.skip(reason="red-phase: 403/captcha → ANTI_BOT mapping")
...
@pytest.mark.skip(reason="red-phase: unknown reason → SOURCE_FAILED")
...
@pytest.mark.skip(reason="red-phase: None reason → SOURCE_FAILED")
...
@pytest.mark.skip(reason="red-phase: all 3 sources degraded → empty items")
...
@pytest.mark.skip(reason="red-phase: 0 sources degraded → not degraded")
...
@pytest.mark.skip(reason="red-phase: degraded=True but items=[] → in source_ids")
...
@pytest.mark.skip(reason="red-phase: degraded=False but items=[] → NOT in source_ids")
...
@pytest.mark.skip(reason="red-phase: len(degraded_source_ids) == count of degraded")
