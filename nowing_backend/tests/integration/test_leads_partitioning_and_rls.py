...
class TestPostgresPartitioningAndRLS:
    ...
    async def test_leads_table_is_partitioned(self, async_db_session: AsyncSession) -> None:
        ...
        shards_res = await async_db_session.execute(
            text("""
    ...
    async def test_composite_foreign_keys_and_cascades(self, async_db_session: AsyncSession) -> None:
        """Verify composite FKs across all 6 dependent child tables (AC-2)."""
        await async_db_session.execute(text("SET LOCAL app.workspace_id = '1'"))
        ...
        """)
        try:
            res = await async_db_session.execute(fks_query)
    ...
    async def test_fail_closed_rls_tenant_isolation(self, async_db_session: AsyncSession) -> None:
        ...
        try:
            ...
            await async_db_session.execute(text(f"SET LOCAL app.workspace_id = '{ws1_id}'"))
            ...
            await async_db_session.execute(text(f"SET LOCAL app.workspace_id = '{ws2_id}'"))
            await async_db_session.execute(
                text("""
                    INSERT INTO leads (id, workspace_id, source, company_name, status)
            ...
            await async_db_session.execute(text("SET LOCAL app.workspace_id = ''"))
            res_empty = await async_db_session.execute(text("SELECT count(*) FROM leads;"))
            ...
            await async_db_session.execute(text(f"SET LOCAL app.workspace_id = '{ws1_id}'"))
            res_ws1 = await async_db_session.execute(text(f"SELECT count(*) FROM leads WHERE id IN ('{lead1_id}', '{lead2_id}');"))
            ...
            await async_db_session.execute(text(f"SET LOCAL app.workspace_id = '{ws2_id}'"))
            res_ws2 = await async_db_session.execute(text(f"SELECT count(*) FROM leads WHERE id IN ('{lead1_id}', '{lead2_id}');"))
    ...
    async def test_partition_pruning_explain_plan(self, async_db_session: AsyncSession) -> None:
        """Verify EXPLAIN plan eliminates 15 of 16 partition shards for workspace query (AC-4)."""
        try:
            await async_db_session.execute(text("SET LOCAL app.workspace_id = '1'"))
            explain_res = await async_db_session.execute(
    ...
    async def test_zero_publication_publish_via_partition_root(self, async_db_session: AsyncSession) -> None:
        """Verify zero_publication has publish_via_partition_root = true (AC-4, INV-23.5)."""
        pub_res = await async_db_session.execute(
