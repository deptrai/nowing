...
async def test_concurrent_payout_approval_prevents_double_spending(
    ...
    assert len(successes) == 1, (
        f"Expected exactly 1 success, got {len(successes)}. Failures: {failures}"
    )
    assert len(failures) == 1, (
        f"Expected exactly 1 conflict failure, got {len(failures)}"
    )
    ...
    async with AsyncSession(async_engine) as verify_session:
        partner_row = (
            (
                await verify_session.execute(
                    text(
                        "SELECT balance_micros, hold_balance_micros FROM affiliate_partners WHERE id = :id"
                    ),
                    {"id": partner_id},
                )
            )
        ...
        payout_row = (
            (
                await verify_session.execute(
                    text(
                        "SELECT status, tx_reference, tax_deducted_micros, net_amount_micros FROM partner_payouts WHERE id = :id"
                    ),
                    {"id": payout_id},
                )
            )
...
async def test_payout_with_pit_10pct_tax_deduction_in_postgres(
    ...
    async with AsyncSession(async_engine) as session:
        ...
        await session.execute(
            text(
                "UPDATE affiliate_partners SET balance_micros = 150000000 WHERE id = :id"
            ),
            {"id": partner_id},
        )
        payout = PartnerPayout(
    ...
    async with AsyncSession(async_engine) as session:
        payout_row = (
            (
                await session.execute(
                    text(
                        "SELECT status, tax_deducted_micros, net_amount_micros, tax_code FROM partner_payouts WHERE id = :id"
                    ),
                    {"id": payout_id},
                )
            )
    ...
    async with AsyncSession(async_engine) as session:
        partner_row = (
            (
                await session.execute(
                    text(
                        "SELECT balance_micros, hold_balance_micros, total_paid_micros FROM affiliate_partners WHERE id = :id"
                    ),
                    {"id": partner_id},
                )
            )
...
async def test_auto_reconciliation_refunds_hold_balance_on_gateway_failure(
    ...
    fake_client.query_transfer_status.return_value = {
        "status": "FAILED",
    ...
    async with AsyncSession(async_engine) as session:
        ...
        assert partner_row["hold_balance_micros"] == 0
        assert (
            partner_row["balance_micros"] == 100_000_000
        )  # 80M refunded back to 20M -> 100M
