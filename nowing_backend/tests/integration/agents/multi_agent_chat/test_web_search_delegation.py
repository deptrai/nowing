...
async def test_web_query_delegates_to_google_search(db_session, db_user, db_workspace):
                ...
                tool_calls=[
                    {
                        ...
                        "args": {
                            ...
                            "description": "latest Nowing release notes",
                        },
                        "id": "call_ws_task",
                    }
                ]
            ),
            ScriptedTurn(text="SERP: Nowing v2 shipped Google-only web search."),
            ScriptedTurn(text="Nowing v2 shipped Google-only web search."),
        ]
    )
    ...
    result = await agent.ainvoke(
        {"messages": [HumanMessage(content="search the web for Nowing news")]},
    ...
    assert _last_ai_text(result["messages"]) == (
        "Nowing v2 shipped Google-only web search."
    )
