...
def _enable_email_gateway(monkeypatch):
    """Enable the email gateway surface for integration tests."""
    from app.config import config
    monkeypatch.setattr(config, "GATEWAY_ENABLED", True)
    monkeypatch.setattr(config, "GATEWAY_EMAIL_ENABLED", True)
    monkeypatch.setattr(config, "GATEWAY_EMAIL_DOMAIN", "nowing.ai")


@pytest.mark.asyncio
async def test_sendgrid_webhook_persists_inbound_email_event(client, db_session, db_user, db_workspace):
    ...
    await set_request_tenant_context(db_session, workspace_id=db_workspace.id)
    row = (
...
@pytest.mark.asyncio
async def test_inbound_email_event_fk_invalid_workspace_raises_integrity_error(
    db_session,
):
    """AC-1 P6: inbound_email_event insert with non-existent workspace_id raises IntegrityError."""
    from sqlalchemy.exc import IntegrityError

    from app.db import InboundEmailEvent, InboundEmailEventStatus

    event = InboundEmailEvent(
        workspace_id=99999,
        provider="sendgrid",
        message_id="<bad-ws@example.com>",
        from_address="test@nowing.net",
        to_address="task+99999@nowing.ai",
        subject="VCB",
        body_text="body",
        status=InboundEmailEventStatus.RECEIVED,
        dedupe_key="sendgrid<bad-ws@example.com>",
    )
