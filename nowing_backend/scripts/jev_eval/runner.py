...
from scripts.jev_eval.cases import ALL_CASES, EvalCase, cases_by_task, get_questions_for_task
...
def score_correctness(case: EvalCase, predicted: Any, backend: str) -> bool:
    ...
    if case.task == "ENTITY_MATCH":
        ...
        try:
            return abs(float(predicted) - float(case.expected)) <= max(case.tolerance, 0.6)
...
async def run_jev(case: EvalCase, client: Any) -> EvalResult:
    ...
    try:
        ...
        return EvalResult(
            case_id=case.id, task=case.task, backend="jev",
            predicted=None, confidence=None, expected=case.expected,
    ...
    try:
        ...
        return EvalResult(
            case_id=case.id, task=case.task, backend="jev",
            predicted=None, confidence=None, expected=case.expected,
            correct=False, latency_ms=(time.perf_counter() - start) * 1000,
    ...
    ans = answers.get(case.question_id) if isinstance(answers, dict) else getattr(answers, case.question_id, None)
    if ans is None:
        return EvalResult(
            case_id=case.id, task=case.task, backend="jev",
            predicted=None, confidence=None, expected=case.expected,
            correct=False, latency_ms=latency,
    ...
    return EvalResult(
        case_id=case.id, task=case.task, backend="jev",
        predicted=predicted, confidence=conf, expected=case.expected,
...
async def run_llm_json(case: EvalCase, model: str = "claude-haiku-4-5-20251001") -> EvalResult:
    """Run a single case through LLM structured output."""
    try:
        import litellm
    except ImportError:
        return EvalResult(
            case_id=case.id, task=case.task, backend="llm_json",
            predicted=None, confidence=None, expected=case.expected,
            correct=False, latency_ms=0.0, error="litellm not installed",
        )
...
def run_mock(case: EvalCase) -> EvalResult:
    ...
    latency = (time.perf_counter() - start) * 1000
    return EvalResult(
        case_id=case.id, task=case.task, backend="mock",
        predicted=predicted, confidence=0.9, expected=case.expected,
...
async def run_all(backends: list[str], task_filter: str | None = None, limit: int | None = None) -> list[EvalResult]:
    cases = cases_by_task(task_filter) if task_filter else ALL_CASES
    if limit:
        cases = cases[:limit]
    logger.info("Running %d cases over backends=%s task=%s", len(cases), backends, task_filter or "all")
    ...
    if "jev" in backends and os.environ.get("TYPESAFE_API_KEY"):
        try:
            ...
            logger.error("typesafe-sdk not installed — run `uv add typesafe-sdk` in nowing_backend")
    ...
    for i, case in enumerate(cases):
        ...
        for backend in backends:
            if backend == "jev" and jev_client:
                ...
                continue
            results.append(r)
            logger.info("  %s → predicted=%r expected=%r correct=%s (%.0fms)",
                        backend, r.predicted, r.expected, r.correct, r.latency_ms)
    return results
...
def write_summary(results: list[EvalResult]) -> None:
    ...
    for (task, backend), rs in sorted(stats.items()):
        ...
        if latencies:
            ...
            acc = n_correct / n
            lines.append(f"| {task} | {backend} | {n} | {acc:.1%} | {med:.0f}ms | {p95:.0f}ms | {errors} |")
        else:
    ...
    for r in results:
        err = (r.error or "")[:40]
        lines.append(f"| {r.case_id} | {r.task} | {r.backend} | `{r.predicted}` | `{r.expected}` | {'✅' if r.correct else '❌'} | {r.latency_ms:.0f}ms | {err} |")
...
async def main() -> int:
    ...
    p.add_argument("--live", action="store_true", help="Run Jev + LLM backends (requires API keys)")
    p.add_argument("--backend", choices=["jev", "llm_json", "mock", "all"], default=None)
    p.add_argument("--task", choices=["SUBAGENT_ROUTING", "ENTITY_MATCH", "CONTENT_FILTER", "INTENT_CLASSIFY"], default=None)
