...
async def run_onedrive_indexing_with_new_session(
    connector_id: int,
    workspace_id: int,
    user_id: str,
    items_dict: dict,
):
    """Wrapper to run Onedrive indexing with its own database session."""
    logger.info(
        f"Background task started: Indexing Onedrive connector {connector_id} into space {workspace_id}"
    )
    ...
    logger.info(
        f"Background task finished: Indexing Onedrive connector {connector_id}"
    )


__all__ = ['run_onedrive_indexing', 'run_onedrive_indexing_with_new_session']
