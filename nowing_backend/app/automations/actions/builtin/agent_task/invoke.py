...
async def _resolve_mention_context(
    ...
    *,
...
) -> tuple[str, NowingContextSchema | None]:
    ...
    build a ``NowingContextSchema`` that the ``search_knowledge_base`` tool
    ...
    # ``NowingContextSchema.mentioned_connectors`` is typed ``list[dict]`` and
    ...
    runtime_context = NowingContextSchema(
        workspace_id=workspace_id,
...
async def run_agent_task(
    *,
    ...
    async with async_session_maker() as agent_session:
        ...
        # The compiled graph declares ``context_schema=NowingContextSchema``;
        ...
        while True:
            ...
            config["configurable"]["nowing_resume_value"] = routed
