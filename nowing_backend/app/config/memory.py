...
# Memory encryption-at-rest (Tier 1). Both the provider and the feature flag
# must be set for encryption to activate; the flag alone is a no-op so a
# staged rollout can ship keys before turning the feature on.
MEMORY_ENCRYPTION_V1 = (
    os.getenv("MEMORY_ENCRYPTION_V1", "false").strip().lower() == "true"
)
NOWING_ENCRYPTION_KEY_PROVIDER = _env_choice(
    "NOWING_ENCRYPTION_KEY_PROVIDER", "none", ("none", "managed", "byok")
)
# Key material. ``none`` provider ignores both; ``managed`` requires the
# master key, ``byok`` requires the customer key. The service layer fails
# closed (raises) when the provider is enabled but its key is missing.
MANAGED_ENCRYPTION_MASTER_KEY = os.getenv("MANAGED_ENCRYPTION_MASTER_KEY")
MANAGED_ENCRYPTION_LEGACY_KEY = os.getenv("MANAGED_ENCRYPTION_LEGACY_KEY")
BYOK_KEY = os.getenv("BYOK_KEY")


__all__ = ["BYOK_KEY", "MANAGED_ENCRYPTION_LEGACY_KEY", "MANAGED_ENCRYPTION_MASTER_KEY", 'MEMORY_AUTO_EXTRACT_BUDGET_MICROS', 'MEMORY_AUTO_EXTRACT_BUDGET_WINDOW', 'MEMORY_AUTO_EXTRACT_CONFIDENCE', 'MEMORY_AUTO_EXTRACT_ENABLED', 'MEMORY_AUTO_EXTRACT_MAX_ITEMS', 'MEMORY_AUTO_EXTRACT_MIN_RESERVE_MICROS', 'MEMORY_AUTO_EXTRACT_RATE_MAX', 'MEMORY_AUTO_EXTRACT_RATE_WINDOW_SECONDS', 'MEMORY_ENCRYPTION_V1', 'NEWS_ENTITY_EXTRACTION_BUDGET_MICROS', 'NEWS_ENTITY_EXTRACTION_BUDGET_WINDOW', 'NEWS_ENTITY_EXTRACTION_CONFIDENCE', 'NEWS_ENTITY_EXTRACTION_ENABLED', 'NEWS_ENTITY_EXTRACTION_MIN_RESERVE_MICROS', 'NEWS_ENTITY_EXTRACTION_RATE_MAX', 'NEWS_ENTITY_EXTRACTION_RATE_WINDOW_SECONDS', 'NEXT_FRONTEND_URL', 'NOWING_ENCRYPTION_KEY_PROVIDER', 'NOWING_PUBLIC_URL']
