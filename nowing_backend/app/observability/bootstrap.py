"""Programmatic OpenTelemetry bootstrap for Nowing backend processes."""
...
def is_otel_disabled() -> bool:
    """Return true when either Nowing or OTel's spec kill switch is set."""
    return _env_truthy("NOWING_DISABLE_OTEL") or _env_truthy("OTEL_SDK_DISABLED")
...
def _deployment_environment() -> str:
    return os.environ.get("NOWING_ENV", "dev")
...
def _build_resource():
    ...
    return Resource.create(
        {
            "service.name": os.environ.get("OTEL_SERVICE_NAME", "nowing-backend"),
...
def _safe_instrument(name: str, instrument: Any) -> bool:
    try:
        instrument()
    except Exception:  # library instrumentation error; log warning and return False
...
def _url_without_query(raw_url: Any) -> str | None:
    ...
    except Exception:  # url parsing error; return None
        return None
...
def init_traces(app: Any | None = None) -> None:
    ...
    except Exception:  # tracer provider registration collision; reuse existing provider
        logger.warning(
...
def init_metrics() -> None:
    ...
    except Exception:  # meter provider registration collision; reuse existing provider
        logger.warning(
