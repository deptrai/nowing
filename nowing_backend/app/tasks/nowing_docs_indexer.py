...
from app.db import NowingDocsChunk, NowingDocsDocument, async_session_maker
...
async def _safe_set_docs_chunks(
    session: AsyncSession, document: NowingDocsDocument, chunks: list
) -> None:
    """safe_set_chunks variant for the NowingDocsDocument/Chunk models."""
    if document.id is not None:
        await session.execute(
            sa_delete(NowingDocsChunk).where(
                NowingDocsChunk.document_id == document.id
            )
        )
        for chunk in chunks:
...
def generate_nowing_docs_content_hash(content: str) -> str:
    """Generate SHA-256 hash for Nowing docs content."""
    return hashlib.sha256(content.encode("utf-8")).hexdigest()


def create_nowing_docs_chunks(content: str) -> list[NowingDocsChunk]:
    """
    Create chunks from Nowing documentation content.

    Args:
        content: Document content to chunk

    Returns:
        List of NowingDocsChunk objects with embeddings
    """
    return [
        NowingDocsChunk(
            content=chunk.text,
...
async def index_nowing_docs(session: AsyncSession) -> tuple[int, int, int, int]:
    ...
    existing_docs_result = await session.execute(
        select(NowingDocsDocument).options(
            selectinload(NowingDocsDocument.chunks)
        )
    )
    ...
    for mdx_file in mdx_files:
        try:
            ...
            content_hash = generate_nowing_docs_content_hash(raw_content)
...
async def seed_nowing_docs() -> tuple[int, int, int, int]:
    """
    ...
    """
    logger.info("Starting Nowing docs indexing...")

    try:
        async with async_session_maker() as session:
            created, updated, skipped, deleted = await index_nowing_docs(session)

        logger.info(
            f"Nowing docs indexing complete: "
        ...
        logger.error(f"Failed to seed Nowing docs: {e}", exc_info=True)
        return 0, 0, 0, 0
