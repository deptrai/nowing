...
class NowingDocsDocument(BaseModel, TimestampMixin):
    """
    Nowing documentation storage.
    Indexed at migration time from MDX files.
    """

    __tablename__ = "nowing_docs_documents"
...
class NowingDocsChunk(BaseModel, TimestampMixin):
    """Chunk storage for Nowing documentation."""

    __tablename__ = "nowing_docs_chunks"
    ...
    document_id = Column(
        Integer,
        ForeignKey("nowing_docs_documents.id", ondelete="CASCADE"),
        nullable=False,
    )
    document = relationship("NowingDocsDocument", back_populates="chunks")
...
class NewLLMConfig(BaseModel, TimestampMixin):
    ...
    - Configurable system instructions (defaults to NOWING_SYSTEM_INSTRUCTIONS)
    ...
    system_instructions = Column(
        Text,
        nullable=False,
        default="",  # Empty string means use default NOWING_SYSTEM_INSTRUCTIONS
    )
    ...
    # Citation toggle - when enabled, NOWING_CITATION_INSTRUCTIONS is injected
...
async def setup_indexes():
    async with engine.begin() as conn:
        ...
        await conn.execute(
            text(
                "CREATE INDEX IF NOT EXISTS idx_nowing_docs_title_trgm ON nowing_docs_documents USING gin (title gin_trgm_ops)"
            )
        )
