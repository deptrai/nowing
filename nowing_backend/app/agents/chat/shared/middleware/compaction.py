"""Nowing compaction middleware.

Extends ``SummarizationMiddleware`` with three Nowing behaviors:

1. A structured summary template (:data:`NOWING_SUMMARY_PROMPT`) instead of
...
NOWING_SUMMARY_PROMPT = """<role>
Nowing Conversation Compaction Assistant
</role>
...
## Relevant Files
What documents or paths in the Nowing knowledge base are in play? Use ``/documents/...`` paths exactly as they appeared in the workspace tree.
</instructions>
...
class NowingCompactionMiddleware(SummarizationMiddleware):
    """SummarizationMiddleware tuned for Nowing.
...
def create_nowing_compaction_middleware(
    model: BaseChatModel,
    ...
    **overrides: Any,
    ...
    """Build a :class:`NowingCompactionMiddleware` with sensible defaults.
    ...
    Args:
        ...
        summary_prompt: Optional override; defaults to :data:`NOWING_SUMMARY_PROMPT`.
        history_path_prefix: Path prefix for offloaded conversation history.
        **overrides: Forwarded to :class:`NowingCompactionMiddleware`.
    """
    defaults = compute_summarization_defaults(model)
    return NowingCompactionMiddleware(
        model=model,
        backend=backend,
        ...
        summary_prompt=summary_prompt or NOWING_SUMMARY_PROMPT,
