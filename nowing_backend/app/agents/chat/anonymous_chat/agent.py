...
deliberately bypass the full Nowing deep agent stack (filesystem,
...
For any other Nowing capability (including web search) the model is
...
def build_anonymous_system_prompt(anon_doc: dict[str, Any] | None = None) -> str:
    ...
    a registration funnel: every other Nowing capability (scraping, live
    ...
    return (
        "You are Nowing's free AI assistant, available to everyone without "
        "login. Nowing is the open-source open web research platform: "
        ...
        "briefly say that this needs a free Nowing account and invite them "
        "to sign up at https://www.nowing.com/register — name the specific "
        "capability that would solve their request (e.g. 'a Nowing account "
        ...
        "## Suggesting Nowing when it genuinely fits\n"
        "You are also the front door to Nowing. When the conversation "
        ...
        "that a free Nowing account can do that with live data, linking "
        "https://www.nowing.com/register.\n"
        ...
        "- Do not suggest it for needs Nowing does not serve (casual chat, "
...
async def create_anonymous_chat_agent(
    *,
    ...
    Unlike :func:`create_nowing_deep_agent`, this agent has no filesystem,
    ...
    middleware: list[Any] = [
        ...
        create_nowing_compaction_middleware(llm, StateBackend),
    ...
    return agent.with_config(
        {
            "recursion_limit": 40,
            "metadata": {
                "ls_integration": "nowing_anonymous_chat",
