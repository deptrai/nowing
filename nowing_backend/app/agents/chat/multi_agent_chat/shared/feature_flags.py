"""Feature flags for the Nowing new_chat agent stack.
...
OFF. The master kill-switch ``NOWING_DISABLE_NEW_AGENT_STACK`` forces every
...
class AgentFeatureFlags:
    ...
    # NOWING_ENABLE_AGENT_CACHE=false.
    ...
    # Rollback via NOWING_ENABLE_CROSS_THREAD_AGENT_CACHE=false.
    ...
    def from_env(cls) -> AgentFeatureFlags:
        ...
        force to False.
        """
        master_off = _env_bool("NOWING_DISABLE_NEW_AGENT_STACK", False)
        if master_off:
            logger.info(
                "NOWING_DISABLE_NEW_AGENT_STACK is set: every new agent "
        ...
        return cls(
            ...
            # Agent quality
            enable_context_editing=_env_bool("NOWING_ENABLE_CONTEXT_EDITING", True),
            ...
            enable_llm_tool_selector=_env_bool(
                "NOWING_ENABLE_LLM_TOOL_SELECTOR", False
            ),
            # Skills + subagents
            enable_skills=_env_bool("NOWING_ENABLE_SKILLS", True),
            enable_specialized_subagents=_env_bool(
                "NOWING_ENABLE_SPECIALIZED_SUBAGENTS", True
            ),
            # Snapshot / revert
            enable_action_log=_env_bool("NOWING_ENABLE_ACTION_LOG", True),
            enable_revert_route=_env_bool("NOWING_ENABLE_REVERT_ROUTE", True),
            # Plugins
            enable_plugin_loader=_env_bool("NOWING_ENABLE_PLUGIN_LOADER", False),
            # Observability
            enable_otel=_env_bool("NOWING_ENABLE_OTEL", False),
            # Performance
            enable_agent_cache=_env_bool("NOWING_ENABLE_AGENT_CACHE", True),
