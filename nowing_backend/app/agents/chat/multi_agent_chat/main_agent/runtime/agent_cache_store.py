"""TTL-LRU cache for compiled Nowing deep agents.

Why this exists
---------------

``create_nowing_deep_agent`` runs a 4-5 second pipeline on EVERY chat
turn:
...
Most middleware in the Nowing stack captures per-thread state in
...
Phase 2 will move those captured fields onto :class:`NowingContextSchema`
...
Every lookup logs ``[agent_cache]`` lines through ``nowing.perf``:
