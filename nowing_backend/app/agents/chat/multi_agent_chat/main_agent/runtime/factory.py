...
async def create_multi_agent_chat_deep_agent(
    ...
    if enabled_tools is not None:
        main_agent_enabled_tools = [
            n for n in enabled_tools if n in MAIN_AGENT_NOWING_TOOL_NAMES
        ]
    else:
        main_agent_enabled_tools = list(MAIN_AGENT_NOWING_TOOL_NAMES_ORDERED)

    _t0 = time.perf_counter()
    # Main agent builds only its own small Nowing toolset via the SRP
