...
def consume_nowing_resume(runtime: ToolRuntime) -> Any:
    """Pop the resume payload for *this* call's ``tool_call_id``.

    The configurable holds ``nowing_resume_value: dict[tool_call_id, payload]``
    ...
    if not isinstance(configurable, dict):
        return None
    by_tcid = configurable.get("nowing_resume_value")
    if not isinstance(by_tcid, dict):
        return None
    payload = by_tcid.pop(runtime.tool_call_id, None)
    if not by_tcid:
        configurable.pop("nowing_resume_value", None)
    return payload


def has_nowing_resume(runtime: ToolRuntime) -> bool:
