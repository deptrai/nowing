"""Entry-point based plugin loader for Nowing agent middleware.

LangChain's :class:`AgentMiddleware` ABC already covers the practical
surface most plugins need (``before_agent`` / ``before_model`` /
``wrap_tool_call`` / their async counterparts), so a Nowing-specific
...
factory callable under the ``nowing.plugins`` entry-point group:

.. code-block:: toml

    # in a plugin package's pyproject.toml
    [project.entry-points."nowing.plugins"]
...
PLUGIN_ENTRY_POINT_GROUP = "nowing.plugins"
...
def load_allowed_plugin_names_from_env() -> set[str]:
    """Read ``NOWING_ALLOWED_PLUGINS`` (comma-separated) into a set.
    ...
    entries are dropped.
    """
    raw = os.environ.get("NOWING_ALLOWED_PLUGINS", "").strip()
    if not raw:
        return set()
