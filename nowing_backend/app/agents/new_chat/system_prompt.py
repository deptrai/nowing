"""
System prompt building for Nowing agents.

This module provides functions and constants for building the Nowing system prompt
...
NOWING_SYSTEM_INSTRUCTIONS = """
<system_instruction>
You are Nowing, a reasoning and acting AI agent designed to answer user questions using the user's personal knowledge base.
...
<system_instruction>
You are Nowing, a reasoning and acting AI agent designed to answer questions in this team space using the team's shared knowledge base.
...
def _get_system_instructions(
    ...
    if visibility == ChatVisibility.SEARCH_SPACE:
        ...
        return NOWING_SYSTEM_INSTRUCTIONS.format(resolved_today=resolved_today)
...
_TOOL_INSTRUCTIONS["search_nowing_docs"] = """
- search_nowing_docs: Search the official Nowing documentation.
  - Use this tool when the user asks anything about Nowing itself (the application they are using).
  - Args:
    - query: The search query about Nowing
...
- generate_podcast: Generate an audio podcast from provided content.
  ...
  - Args:
    ...
    - podcast_title: Optional title for the podcast (default: "Nowing Podcast")
...
- generate_video_presentation: Generate a video presentation from provided content.
  ...
  - Args:
    ...
    - video_title: Optional title (default: "Nowing Presentation")
...
_TOOL_EXAMPLES["search_nowing_docs"] = """
- User: "How do I install Nowing?"
  - Call: `search_nowing_docs(query="installation setup")`
- User: "What connectors does Nowing support?"
  - Call: `search_nowing_docs(query="available connectors integrations")`
- User: "How do I set up the Notion connector?"
  - Call: `search_nowing_docs(query="Notion connector setup configuration")`
- User: "How do I use Docker to run Nowing?"
  - Call: `search_nowing_docs(query="Docker installation setup")`
"""
...
The following tools are available in Nowing but have been disabled by the user for this session: {disabled_list}.
...
NOWING_TOOLS_INSTRUCTIONS = _get_tools_instructions()


NOWING_CITATION_INSTRUCTIONS = """
...
- [citation:doc-123] (for Nowing documentation chunks)
...
NOWING_NO_CITATION_INSTRUCTIONS = """
<citation_instructions>
...
def build_nowing_system_prompt(
    ...
    """
    Build the Nowing system prompt with default settings.
    ...
    citation_instructions = NOWING_CITATION_INSTRUCTIONS
...
def build_configurable_system_prompt(
    ...
    """
    Build a configurable Nowing system prompt based on NewLLMConfig settings.

    The prompt is composed of three parts:
    1. System Instructions - either custom or default NOWING_SYSTEM_INSTRUCTIONS
    2. Tools Instructions - only for enabled tools, with a note about disabled ones
    3. Citation Instructions - either NOWING_CITATION_INSTRUCTIONS or NOWING_NO_CITATION_INSTRUCTIONS
    ...
    citation_instructions = (
        ...
        else NOWING_NO_CITATION_INSTRUCTIONS
    )
...
def get_default_system_instructions() -> str:
    ...
    """
    return NOWING_SYSTEM_INSTRUCTIONS.strip()


NOWING_SYSTEM_PROMPT = build_nowing_system_prompt()
