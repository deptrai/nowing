...
# NOTE: Solana is NOT supported here — GoPlus uses a separate endpoint
# (/api/v1/solana/token_security/) with a different request shape. Add
# dedicated handling if Solana support is required.
...
def create_contract_info_tool():
    ...
    async def get_contract_info(
        ...
        try:
            ...
            logger.warning(
                "Contract info error %s/%s: %s",
                chain, contract_address, type(exc).__name__, exc_info=True,
            )
            return {"error": f"Failed to fetch contract info: {type(exc).__name__}"}
...
def create_check_token_security_tool():
    ...
    async def check_token_security(
        ...
        # Refuse unsupported non-numeric chain names (e.g. "solana") to avoid
        # building malformed URLs like /token_security/solana.
        ...
        try:
            ...
            logger.warning(
                "GoPlus security error %s/%s: %s",
                chain_id, contract_address, type(exc).__name__, exc_info=True,
            )
            return {"error": f"Failed to check token security: {type(exc).__name__}"}
