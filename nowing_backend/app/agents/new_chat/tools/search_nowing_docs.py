...
This tool allows the agent to search the pre-indexed Nowing documentation
...
in dedicated tables (nowing_docs_documents, nowing_docs_chunks).
"""
...
from app.db import NowingDocsChunk, NowingDocsDocument
from app.utils.document_converters import embed_text


def format_nowing_docs_results(results: list[tuple]) -> str:
    """
    ...
    - Frontend to detect 'doc-' prefix and route to nowing docs endpoint
    ...
    """
    if not results:
        return "No relevant Nowing documentation found for your query."
...
async def search_nowing_docs_async(
    query: str,
    db_session: AsyncSession,
    top_k: int = 10,
) -> str:
    """
    Search Nowing documentation using vector similarity.

    Args:
        query: The search query about Nowing usage
    ...
    stmt = (
        select(NowingDocsChunk, NowingDocsDocument)
        .join(
            NowingDocsDocument,
            NowingDocsChunk.document_id == NowingDocsDocument.id,
        )
        .order_by(NowingDocsChunk.embedding.op("<=>")(query_embedding))
        .limit(top_k)
    )
    ...
    return format_nowing_docs_results(rows)


def create_search_nowing_docs_tool(db_session: AsyncSession):
    """
    Factory function to create the search_nowing_docs tool.

    Args:
        db_session: Database session for executing queries

    Returns:
        A configured tool function for searching Nowing documentation
    """

    @tool
    async def search_nowing_docs(query: str, top_k: int = 10) -> str:
        """
        Search Nowing documentation for help with using the application.

        Use this tool when the user asks questions about:
        - How to use Nowing features
        ...
        This searches the official Nowing documentation that was indexed
        at deployment time. It does NOT search the user's personal knowledge base.

        Args:
            query: The search query about Nowing usage or features
        ...
        """
        return await search_nowing_docs_async(
            query=query,
            db_session=db_session,
            top_k=top_k,
        )

    return search_nowing_docs
