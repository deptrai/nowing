...
def _stamp_enqueue_time(headers=None, **_kwargs):
    ...
    with contextlib.suppress(Exception):
        headers["nowing.enqueued_at_ns"] = str(time.monotonic_ns())
...
def _record_queue_latency(task=None, **_kwargs):
    ...
    try:
        ...
        with contextlib.suppress(Exception):
            request.nowing_operation = operation
            request.nowing_queue = queue
            request.nowing_scheduled = scheduled

        headers = getattr(request, "headers", None) or {}
        enqueued_ns = headers.get("nowing.enqueued_at_ns")
        if enqueued_ns is None:
            return

        elapsed_s = (time.monotonic_ns() - int(enqueued_ns)) / 1e9
        with contextlib.suppress(Exception):
            request.nowing_queue_latency_ms = elapsed_s * 1000
...
def _set_celery_span_attributes(task=None, **_kwargs):
    ...
    try:
        ...
        operation = getattr(request, "nowing_operation", None)
        if operation:
