...
const STANDALONE_ROOT = path.join(
  '..', 'nowing_web', '.next', 'standalone', 'nowing_web'
);
  ...
  const shared = {
    ...
    define: {
      'process.env.HOSTED_FRONTEND_URL': JSON.stringify(
        process.env.HOSTED_FRONTEND_URL || desktopEnv.HOSTED_FRONTEND_URL || 'https://nowing.net'
      ),
      ...
      'process.env.POSTHOG_HOST': JSON.stringify(
        process.env.POSTHOG_HOST || desktopEnv.POSTHOG_HOST || 'https://assets.nowing.com'
      ),
    },
  };
