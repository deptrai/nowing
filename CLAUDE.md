# Nowing — Claude Code Guide

## Tech Stack
- Frontend: Next.js 15 + TypeScript + Tailwind (in `nowing_web/`)
- Backend: FastAPI + Python 3.12 + SQLAlchemy (in `nowing_backend/`)
...
- Migrations: Alembic in `nowing_backend/migrations/versions/` — check latest number before creating new one
- Admin guard: use `current_superuser` dependency from `nowing_backend/app/users.py`
...
- Admin/RBAC patterns → `nowing_backend/app/routes/admin_routes.py`
...
- Frontend user/auth atoms → `nowing_web/atoms/user/user-query.atoms.ts`
