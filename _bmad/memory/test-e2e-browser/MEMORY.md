...
- **Local Ports (updated 2026-09-22):** the REAL dev stack uses **host Postgres on `5432`** (`.env` `DATABASE_URL`), backend on **`:4445`** (frontend `.env.local`), Redis on `6380` (`redis-68c` container), frontend `:3000`, Zero `:4848`. Docker `nowing-deps-db-1` on `5434` exists but is STALE — data written there never appears in the app. Older notes saying "Postgres 5434 / backend 8000" describe an earlier setup.
- **Alembic dead revision:** DBs stamped `bcf2ac4cf89d` (merge_235_and_4c04, removed by cleanup commits) break `alembic upgrade`/`stamp` with "Can't locate revision". Fix via direct SQL: `update alembic_version set version_num='239'` (numeric revision ids; head was 239).
- **Playwright MCP arg shape:** `browser_type`/`browser_fill_form` take `target` (the snapshot ref id), not `ref`.
