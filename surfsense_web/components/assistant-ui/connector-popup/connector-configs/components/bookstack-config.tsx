...
export const BookStackConfig: FC<BookStackConfigProps> = ({
	...
	const fetchShelves = useCallback(async () => {
		// Loading guard: prevent multiple simultaneous fetchShelves calls
		if (loadingShelves) return;
	...
	}, [baseUrl, tokenId, tokenSecret, loadingShelves]);
	...
	return (
		<div className="space-y-6">
			...
			<div className="rounded-xl border border-border bg-slate-400/5 dark:bg-white/5 p-3 sm:p-6 space-y-3 sm:space-y-4">
				...
				{shelvesError && <p className="text-xs text-destructive">{shelvesError}</p>}
							...
							return (
								...
								>
									...
									<div className="flex-1 min-w-0">
										<div className="text-xs sm:text-sm font-medium truncate">{shelf.name}</div>
