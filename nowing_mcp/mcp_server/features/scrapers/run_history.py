...
def register(mcp: FastMCP, client: NowingClient, context: WorkspaceContext) -> None:
    ...
    async def list_scraper_runs(
        ...
        nowing_get_scraper_run. Returns each run's id, capability, status,
    ...
    async def get_scraper_run(
        run_id: Annotated[
            str,
            Field(
                description="Run id from nowing_list_scraper_runs or a "
