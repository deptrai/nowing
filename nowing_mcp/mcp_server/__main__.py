...
def main() -> None:
    ...
    transport = os.environ.get("NOWING_MCP_TRANSPORT", "stdio").strip() or "stdio"
    ...
    if transport == "stdio" and not settings.api_key:
        raise SystemExit(
            "NOWING_API_KEY is required for stdio transport. Create an API "
            "key in Nowing (Settings -> API) and pass it via the "
            "NOWING_API_KEY environment variable."
        )
