...
class Settings:
    ...
    def from_env(cls) -> Settings:
        ...
        api_key = os.environ.get("NOWING_API_KEY", "").strip() or None

        base_url = (
            os.environ.get("NOWING_BASE_URL", DEFAULT_BASE_URL).strip().rstrip("/")
        )
        api_prefix = "/" + os.environ.get(
            "NOWING_API_PREFIX", DEFAULT_API_PREFIX
        ).strip().strip("/")

        raw_timeout = os.environ.get("NOWING_TIMEOUT", "").strip()
        try:
            timeout = float(raw_timeout) if raw_timeout else DEFAULT_TIMEOUT_SECONDS
        except ValueError:
            timeout = DEFAULT_TIMEOUT_SECONDS

        default_workspace = os.environ.get("NOWING_WORKSPACE", "").strip() or None

        host = os.environ.get("NOWING_MCP_HOST", "").strip() or DEFAULT_HTTP_HOST
        raw_port = os.environ.get("NOWING_MCP_PORT", "").strip()
        try:
