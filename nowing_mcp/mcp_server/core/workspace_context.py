...
from .client import NowingClient
...
WorkspaceParam = Annotated[
    ...
    Field(
        ...
        "the active workspace (set with nowing_select_workspace)."
    ),
]
...
class Workspace:
    """A Nowing workspace (the product's "search space")."""
...
class WorkspaceContext:
    ...
    def __init__(
        self, client: NowingClient, *, preferred_reference: str | None = None
    ) -> None:
        self._client = client
    ...
    async def _only_workspace_or_prompt(self) -> Workspace:
        ...
        raise ToolError(
            "No workspace selected. Choose one first with nowing_select_workspace, "
