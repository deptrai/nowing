...
from mcp_server.core.client import NowingClient
...
def test_explains_401_with_token_hint():
    message = NowingClient._explain_failure(_response(401, json={"detail": "bad"}))
    assert "API key" in message
    assert "bad" in message


def test_explains_403_as_access_or_api_disabled():
    message = NowingClient._explain_failure(_response(403, json={"detail": "no"}))
...
def test_extracts_nested_detail_message():
    ...
    assert "out of credits" in NowingClient._explain_failure(response)


def test_unmapped_status_still_reports_detail():
    message = NowingClient._explain_failure(_response(500, json={"detail": "boom"}))
    assert "500" in message and "boom" in message


def test_parses_json_body():
    assert NowingClient._parse_body(_response(200, json={"ok": 1})) == {"ok": 1}


def test_empty_body_parses_to_none():
    assert NowingClient._parse_body(_response(204, content=b"")) is None


def test_non_json_body_falls_back_to_text():
    assert NowingClient._parse_body(_response(200, text="hello")) == "hello"
