...
def _client_recording_auth(seen: dict, *, fallback: str | None) -> NowingClient:
...
async def _get(client: NowingClient) -> None:
...
def test_request_key_is_sent_as_bearer():
    ...
    async def run() -> None:
        token = identity.bind_api_key("nw_pat_request")
        try:
            await _get(client)
        finally:
    ...
    asyncio.run(run())
    assert seen["authorization"] == "Bearer nw_pat_request"


def test_request_key_overrides_env_fallback():
    seen: dict = {}
    client = _client_recording_auth(seen, fallback="nw_pat_env")
...
def test_env_fallback_used_without_request_key():
    ...
    assert seen["authorization"] == "Bearer nw_pat_env"
...
def test_concurrent_callers_do_not_leak_keys():
    ...
    async def run() -> None:
        await asyncio.gather(call_as("nw_pat_A"), call_as("nw_pat_B"))

    asyncio.run(run())
    assert seen_by_caller["nw_pat_A"] == "Bearer nw_pat_A"
    assert seen_by_caller["nw_pat_B"] == "Bearer nw_pat_B"
