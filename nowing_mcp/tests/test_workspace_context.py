...
class FakeClient:
    """Stands in for NowingClient, serving a fixed workspace list."""
...
def test_active_workspace_is_isolated_per_identity():
    ...
    asyncio.run(select_as("nw_pat_A", "A"))
    asyncio.run(select_as("nw_pat_B", "B"))

    # Each caller keeps its own selection; no bleed across identities.
    assert asyncio.run(active_for("nw_pat_A")) == 1
    assert asyncio.run(active_for("nw_pat_B")) == 2
    # An unknown caller has no active selection.
    assert asyncio.run(active_for("nw_pat_C")) is None
