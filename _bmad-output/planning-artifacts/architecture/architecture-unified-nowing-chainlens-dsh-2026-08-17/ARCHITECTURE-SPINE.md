...
- **Rule:**
  ...
  2. Contact deduplication MUST use blind HMAC-SHA256 hashes (`value_hmac`). The HMAC input is a single normalized contact string: `HMAC_SHA256("phone={normalized_phone}|email={normalized_email}|domain={domain}", HMAC_SECRET)`. `value_hmac` MUST be `NOT NULL` and part of a `UNIQUE(workspace_id, value_hmac)` constraint.
