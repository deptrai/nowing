...
Multi-agent runtime + memory tools + research continuity. **FRs:** FR-14,15,16,17,42 (+4.5, 4.6). **Open:** 4.7 pointer-based tabs `[ready-for-dev]`, 4.8a chat telemetry `[done]`, 4.8b chat regression suite `[done]`, 4.8c production query sampler + PII anonymizer `[done]`, 4.8d/8e/8g `[ready-for-dev]`, 4.8f benchmark stability `[in-progress]`.
