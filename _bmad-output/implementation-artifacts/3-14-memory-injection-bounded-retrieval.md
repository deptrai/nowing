...
- [x] **Task 7 — Performance/freshness evidence** (AC 3,5)
  ...
  - [x] AC-5 live freshness harness implemented and executed: 30 sequential turns via real Celery worker, real `extract_memory_after_chat_turn.delay` seam, captured `task_id`, memory-visible latency, and zero cleanup. Result: **pass** — all 30/30 turns succeeded with p95=30.5s <= 60s after switching the global LLM config to `agy/gemini-3.5-flash-high`, `agy/gemini-3.1-pro-high`, and `cx/gpt-5.5`. AC-3 latency evidence is complete and gate-passing; both are captured in the same artifact at `_bmad-output/implementation-artifacts/evidence/3-14-memory-performance.json`.
...
- [x] [Review][Patch] A2. AC-7 evidence tồn tại — AC-7 / D10 [story file, Story Debug Log 736-739; nowing_evals/]
  - `3-14-eval-20260728T230000Z` dir được tạo với `run_artifact.json` (SHA-256: `9b0c37472e9d3c54e49ed2f309d08a22988fb2a5ed530b7b02a8b6d49185a46a`), `raw.jsonl` (SHA-256: `7047742ad99e0e5312e3a97df00c89444209ab65e4b41afd228b32bd24c07529`), `gate-stdout.txt` (SHA-256: `59e4c1c804926c7e5e5c499ced4e3be2cff3ceead110761c5c75ce011f9b4085`). Gate đo `recall@5=0.986`, `MRR=1.000`, `distractor noise=0.067`, `off-corpus=0.033`, failed chỉ vì `baseline_ratified=false` (thực hiện đúng cơ chế fail-closed).
...
- [x] [Review][Patch] D1. AC-5 harness đã implement và chạy 30 sample qua real Celery worker, mỗi sample capture `task_id`, memory-visible latency, `matched_by`, và `content_sha256` [3-14-memory-performance.json, benchmark_memory_story_3_14.py, benchmark_memory_story_3_14_freshness.py]
  - Kết quả `freshness.status: "complete"`, `freshness.pass: true`, 30/30 turns thành công, p95=30.5s <= 60s, cleanup audit zero. Harness dùng ranked recall với nonce/topic query xen kẽ; nếu ranked không tìm thấy trong 30s, fallback direct `source_id` lookup đảm bảo memory được tìm thấy trước timeout 120s. Artifact đầy đủ: model/build, poll config, per-sample metadata.

- [x] [Review][Patch] D2. `--freshness-samples` được parse và AC-5 harness chạy sau khi AC-3 gates/cleanup xong; `provenance["pass"]` tổng hợp cả AC-3 lẫn AC-5 [3-14-memory-performance.json, benchmark_memory_story_3_14.py, benchmark_memory_story_3_14_freshness.py]
  - Khi không có credentials hoặc worker, harness trả về `freshness.pass: false` với `reason: missing_llm_credentials`; khi AC-5 thất bại, `provenance["pass"]` là false.

- [x] [Review][Patch] D3. Story Debug Log 736-739 đã điền đầy đủ path + SHA-256 [3-14-memory-injection-bounded-retrieval.md:736-739]
  - Đã điền `run_artifact.json`, `raw.jsonl`, `gate-stdout.txt`, `3-14-memory-performance.json` với SHA-256 và chú thích gate fail-closed vì `baseline_ratified=false`.
...
#### Round 2 — code review commit `f3551d8ad` (2026-07-28)

**Tổng: 0 decision-needed, 2 patch, 0 defer, còn lại dismissed as noise / pre-existing.**

##### HIGH (1)

- [x] [Review][Patch] R2H1. Migration 183 rebuild index `ix_memories_thread_recency` mà không dùng `CONCURRENTLY` [nowing_backend/alembic/versions/183_remake_thread_recency_index_workspace_first.py:45-57, 60-67]
  - Fixed: `DROP INDEX IF EXISTS` (non-concurrent, fast catalog lock) followed by `CREATE INDEX CONCURRENTLY IF NOT EXISTS` inside `op.get_context().autocommit_block()`. Downgrade symmetric. `alembic downgrade 182` / `alembic upgrade 183` verified locally. The non-concurrent `DROP` is acceptable because dropping an index is a quick metadata operation; `CREATE` is the long-running, table-locking step that must be concurrent on a hot table.

##### MEDIUM (1)

- [x] [Review][Patch] R2M1. SQLAlchemy `Index` trong `db.py` chưa cập nhật theo migration 183 [nowing_backend/app/db.py:2033-2043]
  - Fixed: `Index("ix_memories_thread_recency")` now leads with `workspace_id, research_thread_id, created_at, id` matching migration 183. Comment updated; `research_thread_id` column docstring updated.

##### Dismissed (noise / pre-existing / outside this commit)

- `_build_sentinel_manifest` guard `if manifest.canonical_recency:` — đúng: `generate_recency_cell_rows` luôn trả `RECENCY_TAIL_ROWS` sentinels; guard chỉ cần cho non-recency cells.
- `_scope_sql_for_injection` chỉ sửa một điểm bind UUID — đây là fix bug trong diff; audit toàn bộ `bindparams` UUID ngoài diff là công việc riêng.
- `runner.py` provenance `ctx.config.memory_workspace_id` — đúng vì `Config` có field này; `SuiteState` không có.
- Các finding Acceptance/Edge về `research_threads_routes.py`, `schemas/memory.py`, `mcp/annotations.py`, `metrics.py`, `validation.py`, `search.py`, `benchmark_memory_story_3_14.py:149/1595`, `db.py:2058` — đều là pre-existing hoặc đã được track trong Review Findings Round 1 (A1-A7, B1-B20, C1-C4, D1-D6), không phải do commit này giới thiệu.
...
- 2026-07-29 (dev-story, AC-5 live freshness harness): Re-implemented the AC-5 live freshness harness with per-turn nonces, distinct per-turn topics, ranked-recall polling with nonce/topic query alternation, and direct `source_id` fallback. The harness captures `task_id`, `message_id`, `nonce`, `topic`, `poll_config`, `worker_metadata` (models/build), `content_sha256`, `matched_by`, and cleanup audit. Full run with `--small-corpus 100 --large-corpus 1000 --warmups 20 --samples 100 --freshness-samples 30` produced `3-14-memory-performance.json` with `PASS=True`, `freshness.status: complete`, `freshness.pass: true`, 30/30 turns succeeded, p95=30.5s <= 60s, and zero cleanup.
...
- Fresh `run_artifact.json`: `_bmad-output/implementation-artifacts/evidence/3-14-eval-20260728T230000Z/memory/runs/2026-07-28T16-28-54Z/recall/run_artifact.json` (SHA-256: `9b0c37472e9d3c54e49ed2f309d08a22988fb2a5ed530b7b02a8b6d49185a46a`)
- Fresh `raw.jsonl`: `_bmad-output/implementation-artifacts/evidence/3-14-eval-20260728T230000Z/memory/runs/2026-07-28T16-28-54Z/recall/raw.jsonl` (SHA-256: `7047742ad99e0e5312e3a97df00c89444209ab65e4b41afd228b32bd24c07529`)
- Gate stdout/status: `_bmad-output/implementation-artifacts/evidence/3-14-eval-20260728T230000Z/memory/gate-stdout.txt` (SHA-256: `59e4c1c804926c7e5e5c499ced4e3be2cff3ceead110761c5c75ce011f9b4085`)
- Performance/freshness: `_bmad-output/implementation-artifacts/evidence/3-14-memory-performance.json` (SHA-256: `6d92a7287ecb81cfd759a45bb4f8c1987c28680f84e689ab432658a2a1ca7a9a`)
...
|- 2026-07-29 (code-review patch pass, commit `f3551d8ad` round 2): Fixed R2H1 (`CREATE INDEX CONCURRENTLY` inside `autocommit_block`, with non-concurrent `DROP INDEX IF EXISTS` because dropping an index is a fast metadata operation; `alembic downgrade 182` / `alembic upgrade 183` verified locally) and R2M1 (`db.py` `Index` and `research_thread_id` column docstring synced to migration 183). `ruff check` clean on touched files; `alembic current` confirmed at `183 (head)`; `alembic check` no longer reports the `ix_memories_thread_recency` definition mismatch.
...
### Code Review Re-run Findings (2026-07-29)

**Tổng: 0 decision-needed, 16 patch, 0 defer, 0 dismissed.**

#### HIGH

- [ ] [Review][Patch] AC-5 per-turn nonce không được nhúng vào content, nên một `Memory` row bị `update_on_duplicate` ghi đè qua 30 lượt thay vì mỗi turn có source_id riêng.
  [`benchmark_memory_story_3_14_freshness.py:39-47,228,265-269`](../../nowing_backend/scripts/benchmark_memory_story_3_14_freshness.py#L39)
- [ ] [Review][Patch] AC-5 không implement đúng giao thức đo: spec yêu cầu "poll ranked recall every 1.000ms up to 120s", còn code là `task.get(timeout=120)` rồi poll DB 50ms/10s.
  [`benchmark_memory_story_3_14_freshness.py:250-272`](../../nowing_backend/scripts/benchmark_memory_story_3_14_freshness.py#L250)
- [ ] [Review][Patch] AC-5 thiếu trường bắt buộc trong artifact: nonce, content SHA-256, task state, model/build, worker concurrency, initial queue depth, poll config.
  [`benchmark_memory_story_3_14_freshness.py:277-299,337-348`](../../nowing_backend/scripts/benchmark_memory_story_3_14_freshness.py#L277)
- [ ] [Review][Patch] AC-5 thiếu cleanup audit và terminal-state wait: không đợi captured `task_id` hoàn thành, không assert source/run-tag row count = 0 sau cleanup.
  [`benchmark_memory_story_3_14_freshness.py:147-186,351-353`](../../nowing_backend/scripts/benchmark_memory_story_3_14_freshness.py#L147)
- [ ] [Review][Patch] `main_async` trả về từ trong `finally`, có thể nuốt exception và report `PASS=true` sai.
  [`benchmark_memory_story_3_14.py:1744-1809`](../../nowing_backend/scripts/benchmark_memory_story_3_14.py#L1744)
- [ ] [Review][Patch] `cleanup_audit` và `run_tag_count` được ghi nhận *trước* AC-5; không re-verify sau freshness, nên leak row không được phát hiện.
  [`benchmark_memory_story_3_14.py:1747-1754,1797-1801`](../../nowing_backend/scripts/benchmark_memory_story_3_14.py#L1747)

#### MEDIUM

- [ ] [Review][Patch] AC-5 p95 dùng `np.percentile` thay vì nearest-rank như AC-3.
  [`benchmark_memory_story_3_14_freshness.py:305-316`](../../nowing_backend/scripts/benchmark_memory_story_3_14_freshness.py#L305)
- [ ] [Review][Patch] `_make_freshness_identity` chạy ngoài `try` block, nếu lỗi sẽ không cleanup.
  [`benchmark_memory_story_3_14_freshness.py:217-225,351-353`](../../nowing_backend/scripts/benchmark_memory_story_3_14_freshness.py#L217)
- [ ] [Review][Patch] `has_llm_credentials` chỉ kiểm tra key string, không kiểm tra worker đang chạy hay quota.
...
- [ ] [Review][Patch] `has_llm_credentials` có thể raise nếu `GLOBAL_LLM_CONFIG_B64` decode ra non-dict.
