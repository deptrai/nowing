...
- [x] [Review][Patch] Mission Control widget is missing CoT drawer, elapsed time, status label, cancel affordance, total tokens and sparkline — đã thêm CoT collapsible, elapsed time, status badge, cancel button disabled, total tokens, sparkline từ `tokens_used` các subtask. [MissionControlWidget.tsx]
...
- [x] [Review][Patch] Fast-unlock storage uses a single `sessionStorage` map and does not reset TTL on every unlock action — đã chuyển sang `atomFamily` + `atomWithStorage` với key `nowing:fast-unlock-session:{workspaceId}:{userId}`, TTL vẫn reset trên mỗi unlock. [atoms/leads/leads-canvas.atoms.ts, PhoneUnlockPill.tsx, FloatingBulkActionBar.tsx]
