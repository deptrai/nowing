...
- `nowing_web/messages/vi.json` + `messages/en.json` — EDIT: thêm `chainlens` namespace với keys `model_group`, `model_tooltip`.
...
- `nowing_web/components/settings/model-connections/model-provider-connections-panel.tsx` — READ-ONLY verify model picker renders provider groups — không sửa nếu group label dynamic từ `providerDisplay(provider).name`.
...
- Verify model picker UI auto-renders ChainLens group (không cần code change nếu pattern đúng — manual check).
