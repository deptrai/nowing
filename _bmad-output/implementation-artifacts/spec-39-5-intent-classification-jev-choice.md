...
- Live verify script NOT run: no `TYPESAFE_API_KEY` in env/.env. Env guard confirmed working (exits early with clear message). Mock-backend smoke: `classify_intent("Tìm cho tôi quán phở ngon ở Hà Nội")` → `{"label":"search","confidence":0.9,"model":"mock","backend":"mock"}`; whitespace → `None`.
