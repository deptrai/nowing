...
I want the core `leads` table partitioned across 16 hash shards and protected by PostgreSQL engine-level Row-Level Security (RLS), with composite foreign keys and Zero-cache partition-root replication,
...
5. **Phase 5 (RLS & Zero-Cache Reconnect):** Enables and forces Row-Level Security on `leads` and sets `publish_via_partition_root = true` on `zero_publication`.

### AC-2 — Composite Primary & Foreign Key Integrity
**Given** the partitioned `leads` table,
**When** inspected at the database schema level,
**Then** the Primary Key is strictly defined as `PRIMARY KEY (id, workspace_id)`.
**And** the unique deduplication constraint is `UNIQUE (workspace_id, value_hmac)`.
**And** all dependent child tables (`lead_scores`, `verified_contacts`, `zalo_message_logs`, `enrichment_requests`) reference `leads` via composite foreign keys `FOREIGN KEY (lead_id, workspace_id) REFERENCES leads (id, workspace_id) ON DELETE CASCADE`.

### AC-3 — Fail-Closed PostgreSQL Engine Row-Level Security (RLS)
**Given** a database connection with tenant session variable `SET LOCAL app.current_workspace_id = '1'`,
**When** executing `SELECT * FROM leads` without any explicit `WHERE workspace_id = ...` clause,
**Then** PostgreSQL engine-level RLS strictly returns only rows where `workspace_id = 1`.
**And** if `app.current_workspace_id` is empty or unset, the query returns 0 rows (Fail-Closed default).
...
- [ ] **Task 1: Alembic Migration `217_partition_leads_table_zero_downtime.py`**
  ...
  - [ ] Write Phase 5 DDL: RLS policy `leads_workspace_isolation_policy`, `FORCE ROW LEVEL SECURITY`, and `ALTER PUBLICATION zero_publication SET (publish_via_partition_root = true);`.
  - [ ] Implement clean downgrade path (`downgrade()`) restoring unpartitioned structure from `leads_legacy_backup`.

- [ ] **Task 2: SQLAlchemy ORM Model & Relationship Updates**
  - [ ] Update `Lead` model in `nowing_backend/app/db.py`: change `__table_args__` to include composite `PrimaryKeyConstraint('id', 'workspace_id')`.
  - [ ] Update child models (`LeadScore`, `VerifiedContact`, `ZaloMessageLog`, `EnrichmentRequest`) with composite foreign key definitions: `ForeignKeyConstraint(['lead_id', 'workspace_id'], ['leads.id', 'leads.workspace_id'], ondelete='CASCADE')`.
  - [ ] Update SQLAlchemy relationships with `foreign_keys` and `primaryjoin` specifications.
...
- [ ] **Task 3: Zero-Cache Publication & Session Middleware Sync**
  ...
  - [ ] Verify database session manager sets `SET LOCAL app.current_workspace_id = :ws_id` on every transactional query and resets session state upon release.
...
- **INV-23.6 (Fail-Closed RLS Enforcement):** Bắt buộc `ALTER TABLE leads FORCE ROW LEVEL SECURITY;`. Mọi query phải set `app.current_workspace_id`.
