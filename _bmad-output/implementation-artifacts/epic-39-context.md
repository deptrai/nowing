...
- **Entity dedup is two-stage** (speculative fan-out pattern): stage 1 heuristic (spatial/Jaccard dedup, phone/address/image union-find) narrows N entities → K candidate pairs (cap K≤250); stage 2 issues ONE `decide()` call per anchor carrying a `match_decision` Choice over only the heuristic-surviving candidate ids + `no_match` — Jev confirms survivors, never re-scores rejected pairs. ~1 call/anchor regardless of K. Score actions: ≥1.5 auto-merge, 0.5–1.5 curator review queue, <0.5 keep separate. The Jev stage is advisory — heuristic-only dedup still runs when disabled.
- **Verified integration points (source-scanned 2026-09-21):**
  - Entity resolution (39.3): `app/services/dedup/spatial_windowed_dedup.py` (Jaccard/spatial bucket heuristic), `app/services/bds_aggregator/dedupe.py` (phone/address/image union-find across batdongsan/chotot/muaban), `app/services/corporate_verification_service.py` (Levenshtein ≥0.85 fuzzy match — Jev replaces with calibrated Score).
  - Content guardrails (39.4): `app/services/pii/redact.py` (regex-only today), `app/retriever/` (RAG chunk relevance), `app/services/connector_service.py` (scraped-content filter before indexing).
  - Intent classification (39.5): `app/services/chat/` message metadata, `app/services/auto_reply_agent.py` `InboundIntentClassifier` (regex `HOT_INTENT_PATTERNS` today).
  - Voice post-STT (39.6, deferred): `app/services/voice/agent_worker.py` `on_user_turn_completed`.
...
- **Future candidates recorded but out of scope:** lead scoring (`lead_intelligence/scoring/service.py`, may extend 39.3 or become a separate story), hybrid LLM tier selection (`app/services/hybrid_llm_router.py`), sequencer condition evaluation (`app/services/sequencer/service.py`), quality-score enhancement, anti-bot triage, social-copilot outlier semantics.
