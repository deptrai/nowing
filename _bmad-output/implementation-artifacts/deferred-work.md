...
- source_spec: `_bmad-output/implementation-artifacts/spec-39-2-subagent-routing-jev-choice.md`
  ...
  resolved: 2026-09-22 — rescore giờ chạy TRƯỚC cache write và inject `_jev_verdict: true` vào payload khi Jev promote; cả 2 name-cache return sites (normal + breaker-open fallback) OR marker vào `is_verified`. `_dict_to_profile` bỏ qua key lạ nên marker không leak ra CorporateProfile. Consistency thắng scope-boundary: verdict do Jev đưa ra phải stable giữa các reads.
- source_spec: `_bmad-output/implementation-artifacts/spec-39-3-entity-resolution-confidence-scoring.md`
  ...
  resolved: 2026-09-22 — `max_seconds: float | None = None` thêm vào signature; `time.monotonic()` deadline check đầu mỗi anchor iteration → break với `stats.aborted=True` + log số anchors còn lại. Remaining anchors giữ heuristic result. Call-count cap và consecutive-error breaker giữ nguyên.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-39-7-decision-telemetry-dashboard-cost-tracking.md`
  ...
  resolved: 2026-09-22 — `AdminTelemetryService.check_daily_cost_alert()` extract đúng read-path logic (today cost → threshold → `_maybe_insert_cost_alert` với advisory lock + dedupe); task `evaluate_decision_daily_cost_alert` mở `async_session_maker` riêng, beat `crontab(minute="*/15")`, registered trong celery include list.
- source_spec: `_bmad-output/implementation-artifacts/spec-39-7-decision-telemetry-dashboard-cost-tracking.md`
  ...
  resolved: 2026-09-22 — `--persist` flag + `--workspace-id`/`--user-id` (required khi persist); `persist_eval_rows()` ghi `usage_type='decision_eval'` với `call_details={task, backend, model, case_id, correct, eval_run}` — volume/cost metrics vẫn filter `usage_type='decision'` nên eval không pollute prod spend; accuracy query mở rộng `in_(decision, decision_eval)` để count eval ground truth. Mock rows skipped.
- source_spec: `_bmad-output/implementation-artifacts/spec-39-7-decision-telemetry-dashboard-cost-tracking.md`
  ...
  resolved: 2026-09-22 — `DecisionService._record_usage` tự mở `async_session_maker()` khi caller có workspace/user context nhưng không có session (tránh share AsyncSession qua asyncio.gather). Wired context qua: `classify_intent(workspace_id)`, `persist_user_turn` chain, `private_provider` (effective_user_id), new-chat orchestrator/persistence_spawn. Boundary giữ nguyên: connector-search path không có user context → log-only (TokenUsage.user_id NOT NULL).
- source_spec: `_bmad-output/implementation-artifacts/stories/37-1-proactive-intent-signal-radar-background-ingestion-tele.md`
  ...
  resolved: 2026-09-22 — guard giờ check CẢ HAI pools: workspace balance (AC-4 literal) + `wallet_credit.spendable_micros(billing_user_id)` (pool thực sự bị debit). Billing user resolve qua `_resolve_billing_user_id` (subscription creator → owner membership). Wallet read failure fail-open; `billing_user_id=None` → skip billing entirely (không persist signals không bill được).
- source_spec: `_bmad-output/implementation-artifacts/stories/37-1-proactive-intent-signal-radar-background-ingestion-tele.md`
...
- source_spec: `_bmad-output/implementation-artifacts/stories/37-1-proactive-intent-signal-radar-background-ingestion-tele.md`
  ...
  resolved: 2026-09-22 — `_log_stream_health()` chạy mỗi consumer loop: xlen(raw_events) + xpending(group) + xlen(intent_dlq); WARNING khi pending>0 hoặc dlq>0, INFO khi sạch; dict và tuple XPENDING shapes đều handled; metric failure suppressed (không break consumer).
- source_spec: `_bmad-output/implementation-artifacts/stories/37-1-proactive-intent-signal-radar-background-ingestion-tele.md`
...
- source_spec: `_bmad-output/implementation-artifacts/stories/37-2-vietnam-cultural-honorific-and-relationship-tone-engine.md`
  ...
  resolved: 2026-09-22 — `workspace_sender_demographics()` đọc `sequencer_sender_birth_year`/`sequencer_sender_gender` từ `Workspace.icp_criteria` (settings bag sẵn có — không cần migration); wired vào dispatch (sequencer sends), inbound.py (curfew defer context), auto_reply_agent. Workspace keys override env; unset → env fallback; lookup failure fail-open.
- source_spec: `_bmad-output/implementation-artifacts/stories/37-2-vietnam-cultural-honorific-and-relationship-tone-engine.md`
  ...
  resolved: 2026-09-22 — documented intentional exemption: auto-reply trả lời inbound ≠ unsolicited marketing nên không gate. Nếu compliance sau này yêu cầu strict (mọi outbound defer 21:00-08:00) cần queue/deferral design riêng — out of scope.
- source_spec: `_bmad-output/implementation-artifacts/spec-39-6-voice-agent-post-stt-semantic-decisions.md`
  summary: Voice room-metadata orchestrator wiring — `create_call_room` has zero production callers, so `workspace_id`/`user_id`/`session_id` never reach `ctx.room.metadata`; voice telemetry stays log-only and `voice_escalation` context fields stay None until the call dispatcher attaches them.
  evidence: `LiveKitTelephonyClient.create_call_room`/`end_call` have no in-repo call sites (only tests); `VoiceSDRAgent` parses metadata correctly but no producer exists. Settle by wiring the outbound-call dispatcher (epic 38 orchestrator) to attach keys, then verify a `usage_type='decision' task='voice'` TokenUsage row lands.
  status: BLOCKED (story dependency) — 2026-09-24 re-verified: the producer is Story 38.7 "Outbound Trigger Engine" (`sprint-status.yaml: backlog`). The consumer side is done — `VoiceSDRAgent` parses `ctx.room.metadata` → `workspace_id`/`user_id`/`session_id` and degrades to log-only when absent. LiveKit infra now ready: `livekit-dev` Docker container running on :7880 with `LIVEKIT_API_KEY/SECRET` dev keys in `.env.local`; `create_call_room` smoke-tested against the real server — room `call_dev-smoke-test` created with full `{workspace_id, user_id, session_id}` metadata. Nothing in-repo to wire until 38.7 lands; when it does, its `create_call_room(metadata=...)` call completes the path.
- source_spec: `_bmad-output/implementation-artifacts/stories/37-3-smart-meeting-booking-engine-for-auto-reply-google-lark.md`
  summary: RESOLVED — prospect-facing booking page implemented at `nowing_web/app/book/[workspace_id]/[lead_id]/page.tsx` backed by new unauthenticated `app/routes/public_booking_routes.py` (`/api/v1/public/book/{ws}/{lead}/slots|book`, rate-limited, lead-UUID-as-capability). Browser-verified via Chrome MCP on real backend:4445: slots render with ICT labels, invalid lead → 404, book with bad creds → 502 surfaced as VN error + lock released.
  evidence: remaining gap is a real-calendar happy path (needs working Google/Lark creds, not the dummy connector used in E2E).
  resolved: 2026-09-24 — real-calendar happy path verified for BOTH providers on ws46. Google: OAuth connector persisted via `/api/v1/auth/google/calendar/connector/add` (project `Reso`, new web OAuth client, redirect URIs for api.nowing.net + localhost loopback); connector id=151, availability + event create verified live. Lark: self-built app "Nowing Calendar" (`cli_aa3ea7b431385e14`) released v1.0.1 with Bot capability + scopes `calendar:calendar` + `contact:contact.base:readonly`; connector id=152 (encrypted `app_secret`, `lark_user_id`=open_id, `calendar_id`=primary). `resolve_credentials(46)` → both providers; `compute_availability` → 70 merged free slots; `create_event` → real Lark event `9a27872f-…` with VC link `vc-jp.larksuite.com/j/865650401`.
- source_spec: `_bmad-output/implementation-artifacts/stories/37-3-smart-meeting-booking-engine-for-auto-reply-google-lark.md`
  ...
  resolved: 2026-09-24 — verified against live Lark tenant (see entry above). `freebusy/list` and `calendars/{id}/events` both return code:0 with the released app's tenant_access_token. Lark connect flow remains manual provisioning (DB row) — acceptable for single-tenant internal app; multi-tenant OAuth flow would be a separate story if needed.
- source_spec: `_bmad-output/implementation-artifacts/stories/37-6-realtime-prospect-engagement-tracker-and-telegram-alert-bot.md`
  ...
  resolved: 2026-09-23 — reordered exactly per spec: `record_pitch_beacon` now only *prepares* the alert (`_prepare_telegram_alert` → `PreparedPitchAlert{token,peer_id,text}`, pure DB reads); route commits the timeline row first, then `dispatch_prepared_pitch_alert` does the pure-network send, then `mark_pitch_beacon_alerted` flips `details.telegram_alerted` in a second commit (display-only; flag failure rolls back without losing the row). On unsendable/transient failure the Redis cooldown key is released so the next beacon retries. `_dispatch_telegram_alert` kept as a compat wrapper. `expire_on_commit=False` confirmed so post-commit attribute access is safe. Tests updated (51 pass).
- source_spec: `_bmad-output/implementation-artifacts/stories/37-6-realtime-prospect-engagement-tracker-and-telegram-alert-bot.md`
  summary: Live Telegram delivery end-to-end chưa verify (cần bound rep account thật) — user phải redeem pairing code trong Telegram. DNS đã settle bằng `pitch.nowing.net` (không phải `.ai`): Dokploy domain + TLS live, HTTP 200, `PITCH_PORTAL_HOST`/`PITCH_PORTAL_BASE_URL` env set trên cả frontend + backend prod.
  evidence: dispatch logic unit-covered (54 tests); system-account poller `scripts/dev_telegram_system_poller.py` chạy long-poll `@nowingnetbot` (không webhook → không tranh prod). Settle khi user click deep link pairing mới nhất.
  status: PARTIAL — DNS/edge cutover done 2026-09-24 (`pitch.nowing.net` → frontend prod, TLS valid, `/api/:path*` rewrite + CORS + beacon + opt-out + favicon đều sẵn). Telegram delivery RESOLVED 2026-09-24: binding #7 BOUND (peer 6650864876, system acct `nowingnetbot`), `_prepare_telegram_alert` → `dispatch_prepared_pitch_alert` sent a real alert end-to-end (`DISPATCHED: True`). Còn 1 blocker: pitch portal code chỉ có trên `develop` — `origin/production` chưa có pitch files, cần merge/deploy lên production branch trước khi portal serve.
- source_spec: `_bmad-output/implementation-artifacts/stories/37-5-sequencer-to-artifact-integration-1-click-mini-pitch-portal.md`
  ...
  resolved: 2026-09-23 — both legs done. (1) `nowing_web/next.config.ts` adds a `has:[host=pitchHost]` rewrite for `/api/:path*` → `NOWING_BACKEND_INTERNAL_URL` **before** the two-segment portal rewrite (ordering matters or `workspace_slug` would swallow "api") — opt-out/beacon/favicon calls become same-origin, no CORS preflight at all. (2) Fallback for packaged/direct-backend deployments: `_origin_from_url(config.PITCH_PORTAL_BASE_URL)` appended to CORS `allowed_origins` in `app/app/factory.py` and to `_allowed_origins()` in `app/auth/csrf.py` (needed when the session cookie domain covers `*.nowing.ai`). No wildcard credentialed origin introduced.
- source_spec: `_bmad-output/implementation-artifacts/stories/37-5-sequencer-to-artifact-integration-1-click-mini-pitch-portal.md`
  ...
  resolved: 2026-09-23 — `GET /api/v1/public/pitch/favicon?domain=` server-side proxy (`public_pitch_routes.py`): domain strictly validated by `_DOMAIN_RE`, upstream is hardcoded `google.com/s2/favicons` only (no SSRF), Redis-cached 7d, 64KB cap, image/* content-type enforced, rate-limited 120/min. `_prospect_logo_url` now emits the same-origin relative path so it works via the pitch-host Next rewrite or the main-host Caddy proxy — viewer IP never reaches Google. 4 new tests in `test_public_pitch_routes.py` + updated pin in `test_pitch_portal.py` (60 pass).
- source_spec: `_bmad-output/implementation-artifacts/stories/37-5-sequencer-to-artifact-integration-1-click-mini-pitch-portal.md`
  ...
  resolved: 2026-09-23 — `resolve_pitch_workspace_id` now: numeric → direct; published slug wins; unique unpublished slug still resolves (sent links survive unpublish); ambiguous unpublished slug (per-workspace unique constraint allows duplicates) → None, fail closed — composite `(lead_id, workspace_id)` lookup prevents cross-workspace leak. Pinned by updated `TestWorkspaceRefResolution` tests in `test_pitch_engagement.py` (51 pass).
- source_spec: `_bmad-output/implementation-artifacts/stories/37-5-sequencer-to-artifact-integration-1-click-mini-pitch-portal.md`
...
- source_spec: `_bmad-output/implementation-artifacts/stories/37-4-zalo-desktop-web-co-pilot-overlay-in-nowing-lead-clipper.md`
  ...
  resolved: 2026-09-23 — migration `242` adds nullable indexed `signal_events.lead_id` (UUID soft-ref — `leads` PK is composite `(id, workspace_id)` so no single-column FK is possible) + backfill via `DISTINCT ON (workspace_id, lower(company_name))` picking earliest-created lead. `_persist_signal` resolves `lead_id` at write time (earliest match, case-insensitive, same workspace); copilot query prefers `lead_id == lead.id` and falls back to legacy name match only for `lead_id IS NULL` rows. New tests `TestPersistSignalLeadLink` pin both branches (101 pass incl. beacon + copilot suites).
- source_spec: `_bmad-output/implementation-artifacts/stories/37-4-zalo-desktop-web-co-pilot-overlay-in-nowing-lead-clipper.md`
