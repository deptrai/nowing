...
- source_spec: `_bmad-output/implementation-artifacts/stories/37-3-smart-meeting-booking-engine-for-auto-reply-google-lark.md`
  summary: Booking page `/book/{workspace_id}/{lead_id}` has no frontend — `booking_link()` emits a URL with no route in nowing_web, so AC-5 escalation sends prospects a dead link unless `MEETING_BOOKING_BASE_URL` points at an external page.
  evidence: no `/book` route under nowing_web/app; backend `POST /{lead_id}/meeting/book` exists to power it. Settle by building the booking page (or wiring an external scheduler) and E2E-checking the escalation reply.
- source_spec: `_bmad-output/implementation-artifacts/stories/37-3-smart-meeting-booking-engine-for-auto-reply-google-lark.md`
