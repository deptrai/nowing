...
- source_spec: `_bmad-output/implementation-artifacts/stories/37-6-realtime-prospect-engagement-tracker-and-telegram-alert-bot.md`
  ...
  status: PARTIAL — DNS/edge cutover done 2026-09-24 (`pitch.nowing.net` → frontend prod, TLS valid, `/api/:path*` rewrite + CORS + beacon + opt-out + favicon đều sẵn). Còn 2 blockers: (1) Telegram binding — chờ user redeem code `4YdvvB-2` (exp ~19:07 UTC) qua `https://t.me/nowingnetbot?start=4YdvvB-2`; (2) pitch portal code chỉ có trên `develop` — `origin/production` chưa có pitch files, cần merge/deploy lên production branch trước khi portal serve.
