...
- source_spec: `_bmad-output/implementation-artifacts/spec-39-6-voice-agent-post-stt-semantic-decisions.md`
  ...
  status: BLOCKED (story dependency) — 2026-09-23 re-verified: the producer is Story 38.7 "Outbound Trigger Engine" (`sprint-status.yaml: backlog`). The consumer side is done — `VoiceSDRAgent` parses `ctx.room.metadata` → `workspace_id`/`user_id`/`session_id` and degrades to log-only when absent. Nothing in-repo to wire until 38.7 lands; when it does, its `create_call_room(metadata={workspace_id, user_id, session_id})` call completes the path.
- source_spec: `_bmad-output/implementation-artifacts/stories/37-3-smart-meeting-booking-engine-for-auto-reply-google-lark.md`
  ...
  status: BLOCKED (external) — cần Google Calendar OAuth tokens / Lark tenant thật; không phải code gap.
- source_spec: `_bmad-output/implementation-artifacts/stories/37-3-smart-meeting-booking-engine-for-auto-reply-google-lark.md`
  ...
  status: BLOCKED (external) — cần Lark tenant + OAuth app credentials; code path đã sửa theo docs nhưng không verify được khi chưa có tenant.
...
- source_spec: `_bmad-output/implementation-artifacts/stories/37-6-realtime-prospect-engagement-tracker-and-telegram-alert-bot.md`
  ...
  status: BLOCKED (external) — Telegram delivery cần rep account thật bind bot; DNS `pitch.nowing.ai` → web là infra/edge cutover ngoài repo. Code path (rewrite + CORS + beacon + opt-out + favicon) đã implement đầy đủ 2026-09-23 — chỉ chờ DNS trỏ tới để active.
