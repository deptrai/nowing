diff --git a/nowing_backend/.env.example b/nowing_backend/.env.example
index 903baa59c..62f0e259d 100644
--- a/nowing_backend/.env.example
+++ b/nowing_backend/.env.example
@@ -83,13 +83,15 @@ SECRET_KEY=SECRET
...
-# Docker. In cloud, use .nowing.com so api., zero., and the app share the
...
@@ -287,6 +289,14 @@ MICROS_PER_PAGE=1000
...
+        return len(self.items)
diff --git a/nowing_backend/app/capabilities/core/billing.py b/nowing_backend/app/capabilities/core/billing.py
index 725ec3597..d5c709f7c 100644
--- a/nowing_backend/app/capabilities/core/billing.py
+++ b/nowing_backend/app/capabilities/core/billing.py
@@ -46,6 +46,9 @@ _PLATFORM_RATE_KEYS: dict[BillingUnit, str] = {
...
@@ -69,6 +72,9 @@ _UNIT_NOUNS: dict[BillingUnit, str] = {
...
diff --git a/nowing_backend/app/capabilities/core/types.py b/nowing_backend/app/capabilities/core/types.py
index b30212627..58d2d0cb0 100644
--- a/nowing_backend/app/capabilities/core/types.py
+++ b/nowing_backend/app/capabilities/core/types.py
@@ -32,6 +32,9 @@ class BillingUnit(StrEnum):
...
diff --git a/nowing_backend/app/mcp_tools.py b/nowing_backend/app/mcp_tools.py
index 43866adea..38f56e4e2 100644
--- a/nowing_backend/app/mcp_tools.py
+++ b/nowing_backend/app/mcp_tools.py
@@ -25,6 +25,9 @@ MCP_TOOL_CATALOG: list[dict[str, str]] = [
...
+
+    # Fallback: a plain phone text span (often a support number, not the agent).
...
+            await asyncio.sleep(_page_delay())
+
+    # Best-effort: resolve detail URLs from the web listing pages and then
+    # fetch phone numbers from each detail page.  Full phone numbers are
+    # usually gated by login/OTP, so we keep the masked display on failure.
+    if resolve_phones and AsyncStealthySession is not None:
+        try:
...
index ae4fc9cf2..045f6f184 100644
...
@@ -21,6 +24,9 @@ from .admin_global_model_connections_routes import (
...
@@ -144,6 +150,9 @@ router.include_router(
...
diff --git a/nowing_backend/app/routes/admin_scraper_platform_accounts_routes.py b/nowing_backend/app/routes/admin_scraper_platform_accounts_routes.py
new file mode 100644
index 000000000..b8e1a0f2c
--- /dev/null
+++ b/nowing_backend/app/routes/admin_scraper_platform_accounts_routes.py
@@ -0,0 +1,205 @@
...
+
+
+def _parse_cookie_input(cookie_input: str | list[dict[str, Any]]) -> list[dict[str, Any]]:
...
+    return [
+        {"name": key, "value": morsel.value, "domain": ".batdongsan.com.vn", "path": "/"}
...
+    so ``detail_url`` is only populated when ``resolve_phones=True`` triggers
+    the web-listing resolver.
+    """
+    output = await scrape_batdongsan(
+        BatdongsanScrapeInput(
+            listing_type="buy",
...
+    assert output.degraded is True
+    assert output.degradation_reason == "empty"
diff --git a/nowing_mcp/mcp_server/features/scrapers/__init__.py b/nowing_mcp/mcp_server/features/scrapers/__init__.py
index 92d76568d..5334fb11f 100644
--- a/nowing_mcp/mcp_server/features/scrapers/__init__.py
+++ b/nowing_mcp/mcp_server/features/scrapers/__init__.py
...
diff --git a/nowing_mcp/mcp_server/features/scrapers/platforms/batdongsan.py b/nowing_mcp/mcp_server/features/scrapers/platforms/batdongsan.py
new file mode 100644
index 000000000..f2ab22abc
--- /dev/null
+++ b/nowing_mcp/mcp_server/features/scrapers/platforms/batdongsan.py
@@ -0,0 +1,95 @@
...
diff --git a/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/platforms/batdongsan/__pycache__/test_scraper.cpython-312-pytest-9.0.2.pyc b/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/platforms/batdongsan/__pycache__/test_scraper.cpython-312-pytest-9.0.2.pyc
new file mode 100644
index 000000000..3742e317f
Binary files /dev/null and b/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/platforms/batdongsan/__pycache__/test_scraper.cpython-312-pytest-9.0.2.pyc differ

diff --git a/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/platforms/batdongsan/__pycache__/test_fetch_web.cpython-312-pytest-9.0.2.pyc b/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/platforms/batdongsan/__pycache__/test_fetch_web.cpython-312-pytest-9.0.2.pyc
new file mode 100644
index 000000000..2c4851e27
Binary files /dev/null and b/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/platforms/batdongsan/__pycache__/test_fetch_web.cpython-312-pytest-9.0.2.pyc differ

diff --git a/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/platforms/batdongsan/__pycache__/test_parsers.cpython-312-pytest-9.0.2.pyc b/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/platforms/batdongsan/__pycache__/test_parsers.cpython-312-pytest-9.0.2.pyc
new file mode 100644
index 000000000..c4cc9a1b3
Binary files /dev/null and b/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/platforms/batdongsan/__pycache__/test_parsers.cpython-312-pytest-9.0.2.pyc differ

diff --git a/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/platforms/batdongsan/__pycache__/test_fetch_decode.cpython-312-pytest-9.0.2.pyc b/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/platforms/batdongsan/__pycache__/test_fetch_decode.cpython-312-pytest-9.0.2.pyc
new file mode 100644
index 000000000..1a571d997
Binary files /dev/null and b/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/platforms/batdongsan/__pycache__/test_fetch_decode.cpython-312-pytest-9.0.2.pyc differ

diff --git a/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/capabilities/batdongsan/scrape/__pycache__/test_schemas.cpython-312-pytest-9.0.2.pyc b/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/capabilities/batdongsan/scrape/__pycache__/test_schemas.cpython-312-pytest-9.0.2.pyc
new file mode 100644
index 000000000..97338962f
Binary files /dev/null and b/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/capabilities/batdongsan/scrape/__pycache__/test_schemas.cpython-312-pytest-9.0.2.pyc differ

diff --git a/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/capabilities/batdongsan/scrape/__pycache__/test_executor.cpython-312-pytest-9.0.2.pyc b/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/capabilities/batdongsan/scrape/__pycache__/test_executor.cpython-312-pytest-9.0.2.pyc
new file mode 100644
index 000000000..9df987526
Binary files /dev/null and b/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/capabilities/batdongsan/scrape/__pycache__/test_executor.cpython-312-pytest-9.0.2.pyc differ

diff --git a/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/capabilities/batdongsan/scrape/__pycache__/test_billing.cpython-312-pytest-9.0.2.pyc b/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/capabilities/batdongsan/scrape/__pycache__/test_billing.cpython-312-pytest-9.0.2.pyc
new file mode 100644
index 000000000..d9138d50e
Binary files /dev/null and b/Users/luisphan/Documents/GitHub/nowing/nowing_backend/tests/unit/capabilities/batdongsan/scrape/__pycache__/test_billing.cpython-312-pytest-9.0.2.pyc differ
