...
- [x] [Resolved][Defer] **CHOTOT_ITEM billing mapping in `billing.py`** [`billing.py:51,88,217`] — Mapping is correct and required by `app/capabilities/chotot/scrape/definition.py`; ownership belongs to Stories 10.6/10.7 but is harmless in 20.4.
- [x] [Resolved][Defer] **Distributed lock for token rotation in multi-instance deployments** [`auth.py:126-157`] — Single-instance `threading.Lock` is now in place. Cross-instance consistency requires a shared `ChainLensServiceToken` store; documented as a `ponytail:` for future work.
- [x] [Resolved][Defer] **JWT signature validation in `_token_expiry`** [`auth.py:106-124`] — By design: tokens are trusted by exact match against the configured pool; `exp` is only used for best-effort proactive rotation, so signature validation is unnecessary.
- [x] [Resolved][Defer] **TokenUsage `usage_type` constants for future ChainLens operations** [`token_tracking_service.py:54-69`] — Added `UsageType` StrEnum with `DEEP_RESEARCH`, `CHAINLENS_INGEST`, `CHAINLENS_GAP_FILL`, `CHAINLENS_PRIVATE_SEARCH` and other canonical values; `billing.py` now uses `UsageType.DEEP_RESEARCH`.
- [x] [Resolved][Defer] **Integration test asserting `cost_dollars_to_micros` is used end-to-end** [`test_executor.py:836-853`] — Added `test_sse_parser_cost_micros_uses_chainlens_auth_half_up_converter` and `test_sse_parser_invalid_cost_dollars_returns_none`.
