...
- [x] [Review][Resolve] **F1 — Entity filter via separate `entity`/`entity_type` fields**: Confirmed ChainLens `SearchRestRequestDto` (Python SDK `SearchRequest`) has no `entity`/`entity_type` fields; disambiguation is encoded into `query` plus `category: news`. Added code comment in executor. Query-only filter is the accepted contract boundary.
- [x] [Review][Resolve] **F2 — PII redaction of ChainLens returned snippets**: `_parse_entity_sources` now runs `redact_pii(..., context="news_ner")` on title/snippet before returning, removing phone/email PII. Names are intentionally preserved because `news_ner` does not mask names (public web results); engine remains responsible for any deeper person-name redaction.
